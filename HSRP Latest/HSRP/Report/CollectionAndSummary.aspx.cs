...
{
public partial class CollectionAndSummary : System.Web.UI.Page
{
    int UserType1;
    ...
    protected void btnexport_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            try
            {
                ...
                book.Properties.Title = "HSRP Total Vehicle Count Collection";
                ...
                Worksheet sheet = book.Worksheets.Add("HSRP Total Vehicle Count Collection");
                ...
                WorksheetCell cell = row.Cells.Add("HSRP Total Vehicle Count Collection");
                ...
                row.Cells.Add(new WorksheetCell("", "HeaderStyle2"));
                            ...
                            if (recordtype =="Both")
                            {
                                SQLString1 = " select (select rtolocationname from rtolocation where rtolocationid=a.rtolocationid) locationname,"
                                                ...
                                                + " SUM(CASE WHEN (VehicleType='SCOOTER')  THEN 1 ELSE 0 END ) AS [Scooter],"
                                                + "  SUM(CASE WHEN (VehicleType='MOTOR CYCLE')  THEN 1 ELSE 0 END ) AS [MC],"
                                                + "SUM(CASE WHEN (VehicleType = 'Three Wheeler') THEN 1 ELSE 0 END ) AS [3W],"
                                                + "SUM(CASE WHEN (VehicleType = 'Tractor' )  THEN 1 ELSE 0 END ) AS [Tractor],"
                                                + "SUM(CASE WHEN (VehicleType = 'LMV')   THEN 1 ELSE 0 END ) AS [4W],"
                                                + " SUM(CASE WHEN (VehicleType='LMV(Class)')  THEN 1 ELSE 0 END ) AS [4WClass],"
                                                + "SUM(CASE WHEN (Vehicletype ='MCV/HCV/TRAILERS')  THEN 1 ELSE 0 END ) AS [Heavy] , "
                                                + "SUM(CASE WHEN Vehicletype in ('SCOOTER','MOTOR CYCLE','Three Wheeler','Tractor','LMV','LMV(Class)','MCV/HCV/TRAILERS') THEN 1 ELSE 0 END ) AS [Total]"
                                                + " from HSRPRecords a where a.HSRP_StateID=" + DropDownListStateName.SelectedValue.ToString() + " and  a.HSRPRECORD_CREATIONDATE between '" + ReportDate1 + "' and '" + ReportDate2 + "' AND netamount >0 "
                            ...
                            else if (OrderType == "Normal")
                            {
                                SQLString1 = " select (select rtolocationname from rtolocation where rtolocationid=a.rtolocationid) locationname,"
                                        ...
                                        + "SUM(CASE WHEN (Vehicletype ='MCV/HCV/TRAILERS')  THEN 1 ELSE 0 END ) AS [Heavy] , "
                                        + "SUM(CASE WHEN Vehicletype in ('SCOOTER','MOTOR CYCLE','Three Wheeler','Tractor','LMV','LMV(Class)','MCV/HCV/TRAILERS') THEN 1 ELSE 0 END ) AS [Total]"
                                        + " from HSRPRecords a where  a.HSRP_StateID=" + DropDownListStateName.SelectedValue.ToString() + " and  a.HSRPRECORD_CREATIONDATE between '" + ReportDate1 + "' and '" + ReportDate2 + "' AND netamount >0 AND a.addrecordby in (" + recordtype + ")"
                                        + " group by a.rtolocationid  "
                                        + " order by (select rtolocationname from rtolocation where rtolocationid=a.rtolocationid) ";
