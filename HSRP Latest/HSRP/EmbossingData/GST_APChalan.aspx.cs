        ...
        public void Showgridfun()
        {
            try
            {
                ...
                try
                {
                    ...
                    //string strtaxamt = "SELECT * from tax where hsrp_stateid='" + HSRPStateID + "' and isnull(GSTBasicAmount,0)!=0";
                    //DataTable dttaxamt = Utils.GetDataTable(strtaxamt, CnnString);
                ...
                string SQlQuerytax = "select distinct a.SGSTPer,a.SGSTAmount,a.CGSTPer,a.CGSTAmount,a.GSTBasicAmount from hsrprecords a , tax b where a.ordertype=b.ordertype and a.hsrp_stateid='" + HSRPStateID + "' and b.OrderType in ('NB','DB','DF','DR','OB') and a.HsrpRecordID in(" + strHsrpRecordId + ")";
                DataTable dt1 = Utils.GetDataTable(SQlQuerytax, CnnString);

                //...................Add Dealer Name ................................
                string strdname = string.Empty;

                string strdealer = "select dealerid, dealername from dealermaster where dealerid = (select distinct dealerid from hsrprecords where challanno ='"+strInvoiceNo+"')";
                ...
                if (true)
                {
                    ...
                    //decimal tax = decimal.Round(Convert.ToDecimal(dataSetFillHSRPDeliveryChallan.Rows[0]["SGSTPer"].ToString()));
                    //decimal tax1 = decimal.Round(Convert.ToDecimal(dataSetFillHSRPDeliveryChallan.Rows[0]["CGSTPer"].ToString()));
                    ...
                    //decimal k = Convert.ToDecimal(Newamount);
                    //decimal one = k / 128;
                    //decimal amt = one * 100;
                    //decimal tax = amt / 100 * cgstpre;
                    //decimal tax1 = amt / 100 * sgstpre;
                    //amt = decimal.Round(amt, 2, MidpointRounding.AwayFromZero);
                    //tax = decimal.Round(tax, 2, MidpointRounding.AwayFromZero);
                    //string strcgstamb =dt.Rows[0]["CGSTAmount"].ToString();
                    //string strsgstamb =dt.Rows[0]["SGSTAmount"].ToString();
