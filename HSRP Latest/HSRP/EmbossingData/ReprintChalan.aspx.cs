        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            ...
            {                
                   ...
                   if (DropDownListStateName.SelectedValue.ToString() == "1")
                  {
                      ...
                      strFrmDateString + "' and  '" + strToDateString + "'  and  convert(date,hsrprecord_creationdate) >'12-jan-2017'  ";
             ...
             //   Button2.Visible = true;
               ...
               // SQlQuery = "select vehicletype+' (Damage Front)' as DescriptionOfGoods,count(*) as qty,round((sum(EXCISEBASIC)/count(*)),3) amt, hsrpweight*count(*) as HSRPWeight,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess  from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and ordertype in ('DF') and HsrpRecordID in(" + strHsrpRecordId + ") group by vehicletype union   select vehicletype+' (Damage Rear)',count(*) as qty,round((sum(EXCISEBASIC)/count(*)),2) amt,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in(" + strHsrpRecordId + ") and  ordertype in ('DR') group by vehicletype union select vehicletype+' (Both Plates)',(count(*)*2)/2 qty,round((sum(EXCISEBASIC)/count(*)),2) amt,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in(" + strHsrpRecordId + ") and ordertype in ('NB','OB','DB') group by vehicletype";
                ...
                if (true)
                {
                    ...
                    //GenerateCell(table, 8, 1, 1, 0, 0, 2, 1, "", 0, 0);
                    ...
                    string strdealer = "select dealerid, dealername from dealermaster where  dealerid in (select distinct Dealerid from hsrprecords  where HSRP_StateID='" + DropDownListStateName.SelectedValue.ToString()+ "' and  ChallanNo='" + dropdownDuplicateFIle.SelectedValue.ToString() + "'  )";
