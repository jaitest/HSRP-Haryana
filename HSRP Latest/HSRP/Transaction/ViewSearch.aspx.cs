    ...
    public partial class ViewSearch : System.Web.UI.Page
    {
        ...
        protected void button_Click(object sender, EventArgs e)
        {
            ...
            if (RadioBtnchassisnosearch.Checked == true)
            {
                try
                {
                    SQLString = "SELECT top 20 [HSRPRecordID],[HSRP_StateID],CONVERT(varchar, HSRPRecords.OrderDate,105)  as OrderDate,convert(varchar, HSRPRecord_AuthorizationDate, 105) as HSRPRecord_AuthorizationDate, HSRPRecord_AuthorizationNo,convert(varchar, OrderEmbossingDate, 105) as OrderEmbossingDate,HSRP_Front_LaserCode, HSRP_Rear_LaserCode, convert(varchar, OrderClosedDate, 105) as InvoiceDateTime, ChassisNo,HSRPRecord_AuthorizationNo,VehicleRegNo,NetAmount,[OwnerName] ,EngineNo,VehicleType,[MobileNo],[EmailID],OrderStatus FROM [HSRPRecords] Where hsrp_stateID='" + HSRPStateID + "' and chassisno like '%" + txtSearchAll.Text + "%'  order by HSRPRecord_CreationDate desc";
                    DataTable dt = Utils.GetDataTable(SQLString, CnnString);
                    Grid1.DataSource = dt;
                    Grid1.RunningMode = (ComponentArt.Web.UI.GridRunningMode)Enum.Parse(typeof(ComponentArt.Web.UI.GridRunningMode), "Client");
                    Grid1.SearchOnKeyPress = true;
        ...
        protected void RadioButtonVehicleRegNoSearch_CheckedChanged(object sender, EventArgs e)
        {
