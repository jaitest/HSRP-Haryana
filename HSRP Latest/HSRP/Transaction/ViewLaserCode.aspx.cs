...
{
    public partial class AssignLaserCode : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                RTOLocationID = Convert.ToInt32(Session["RTOLocationID"].ToString());
        ...
        private void FilldropDownListClient()
        {
            if (UserType.Equals(0))
            {
                ...
                SQLString = "select RTOLocationName,RTOLocationID from RTOLocation Where HSRP_StateID=" + intOrgID + "Order by RTOLocationName";
            }
            else
            {
        ...
        public void buildGrid()
        {
            try
            {
                if (UserType.Equals(0))
                {
                    ...
                    SQLString = "SELECT HSRPRecordID,HSRPRecord_AuthorizationNo,OwnerName,MobileNo,EmailID,ChassisNo,EngineNo,VehicleRegNo FROM [HSRPRecords]  order by HSRPRecord_AuthorizationNo";
                }
                else if (UserType.Equals(1))
                {
                    intOrgID = HSRPStateID;
