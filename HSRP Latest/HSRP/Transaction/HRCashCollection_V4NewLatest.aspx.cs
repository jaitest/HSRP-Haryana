...
{
    public partial class HRCashCollection_V4NewLatest : System.Web.UI.Page
    {
          ...
          //  btnDownload.Visible = false;
            ...
            try
            {
                ...
                if (dtbl.Rows.Count > 0)
                {
                    ...
                    string VehicleNo = Convert.ToString(txtRegNo.Text).Trim();
                ...
                if (HiddenField1.Value != "")
                {
                    if (txtAC.Text != "")
                    {
                        String SqlAc = "Select h.acno from HRAC_master h where h.hsrp_stateid='" + HSRPStateID + "' and h.userid='" + USERID + "' and h.acno='" + txtAC.Text.Trim() + "' and h.activestatus='Y'";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        ddlVehicleClass.Visible = true;
                        lblVehicleClassType.Visible = false;

                       if (!string.IsNullOrEmpty(dt.Rows[0]["RegistrationDate"].ToString().Trim()))
                            {
                                OrderDate.SelectedDate = Convert.ToDateTime(dt.Rows[0]["RegistrationDate"].ToString().Trim());
                            }
                    ...
                    {
                        ddlVehicleClass.Visible = false;
                        lblVehicleClassType.Visible = true;
        ...
        {
            String SqlQuerycheck = "SELECT top(1) hsrpflag1 FROM HSRPRecordsStaggingArea where VehicleRegNo='" + txtRegNo.Text + "' and HSRP_StateID='" + HSRPStateID + "' order by  HSRPRecord_CreationDate desc";
            DataTable dtcheck = Utils.GetDataTable(SqlQuerycheck, ConnectionString);
            if (dtcheck.Rows[0]["hsrpflag1"].ToString().Trim() == "V4")
            {
                ...
                try
                {
                    ...
                    if (ddltakal.SelectedValue.ToString() == "--Select Tatkal Type--")
                    {

                        lblErrMess.Visible = true;
                        lblErrMess.Text = "Please Select Tatkal Type .";
                        return;
                    }
                    ...
                    string sqlqueryDuplicate = "select count (chassisno)as noo from hsrprecords where  hsrp_stateid = 4 and  LTRIM(RTRIM(chassisno))= '" + lblChasisNo.Text.ToString().Trim() + "' and  LTRIM(RTRIM(ordertype))= 'NB' ";
                    int noo = Utils.getScalarCount(sqlqueryDuplicate, ConnectionString);
                    ...
                    if (OrderDate.SelectedDate.ToString() == "" || OrderDate.SelectedDate.ToString() == null || OrderDate.SelectedDate.ToString() == "01/01/0001 00:00:00" || OrderDate.SelectedDate.ToString() == "01/01/0001 12:00:00 AM")
                    {
                        lblErrMess.Visible = true;
                    ...
                    cmd.Parameters.AddWithValue("@Reference", ddltakal.SelectedValue.ToString());
            ...
            else
            {
                try
                {
                    ...
                    if (lblAmount.Text.ToString().Trim() == "0")
                    {
                        lblErrMess.Visible = true;
                        lblErrMess.Text = "Please  Contact to Administor.";
                        return;
                    ...
                    if (message == "Y")
                    {
                       ...
                       // btnDownload.Visible = true;
                        ...
                        if (txtMobileno.Text.Length > 0)
                        {
                            ...
                            string SMSText = "Received Rs. " + Math.Round(decimal.Parse(lblAmount.Text), 0) + " against HSRP For Reg No. " + lblRegNo.Text.Trim().ToUpper() + " RECEIPT No. " + cashrc + " dated  " + System.DateTime.Now.ToString("dd/MM/yyyy") + "-HSRP Team ";
                            string sendURL = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-hsrphr&password=hsrphr&type=0&dlr=1&destination=" + txtMobileno.Text.ToString() + "&source=HRHSRP&message=" + SMSText;
                            // string sendURL = "http://onextelbulksms.in/shn/api/pushsms.php?usr=demolinkutsav&pwd=Look@123&sndr=HRHSRP&ph="+ txtMobileno.Text.ToString().Trim()+"&text=" + SMSText + "&rpt=1";
