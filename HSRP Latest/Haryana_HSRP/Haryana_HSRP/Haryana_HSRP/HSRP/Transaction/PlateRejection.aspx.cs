...
namespace HSRP.Master
{
    public partial class PlateRejection : System.Web.UI.Page
    {
        ...
        protected void buttonRejectedSave_Click(object sender, EventArgs e)
        {
            ...
            sqlstring = "select top 1 orderstatus from hsrprecords where vehicleregno='" + txtVehicleNo.Text + "' order by HSRPRecord_CreationDate desc";
            DataTable dt2 = Utils.GetDataTable(sqlstring, CnnString);
            if(dt2.Rows.Count > 0)
            {
                if (dt2.Rows[0]["orderstatus"].ToString() == "Closed")
                {
                    lblErrMess.Text = "";
                    lblSucMess.Text = "";
           ...
           // sqlstring = "select * from rejectplatEntry where hsrp_stateid='" + HSRPStateID + "' and  RearLaserCode='" + lblRLNo.Text + "' ";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            SQLString = "select  top 1  orderstatus, OrderType,HSRP_Front_LaserCode,HSRP_Rear_LaserCode,HSRPRecordID,rtolocationID,HSRP_Stateid from HSRPRecords where VehicleRegNo='" + txtVehicleNo.Text + "' and  ( orderstatus='Embossing Done' or orderstatus='New Order' ) order by HSRPRecord_CreationDate desc";
            DataTable dt = Utils.GetDataTable(SQLString, CnnString);           
