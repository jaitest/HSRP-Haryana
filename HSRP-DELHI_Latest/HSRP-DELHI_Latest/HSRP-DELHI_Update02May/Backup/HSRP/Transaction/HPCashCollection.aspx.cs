...
{
    public partial class HPCashCollection : System.Web.UI.Page
    {
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                String SqlQuery = "SELECT HSRPRecord_AuthorizationNo,RegistrationDate,OwnerName,Address1,HSRP_FLAG,VehicleClass,NICvehicletype,MAKER,MAKER_MODEL,ChassisNo,EngineNo,VehicleRegNo FROM HP_StaggingArea where VehicleRegNo='" + txtRegNo.Text.Trim() + "' and HSRP_StateID='" + HSRPStateID + "'";
                DataTable dt = Utils.GetDataTable(SqlQuery, ConnectionString);
                if (dt.Rows.Count > 0)
                {
                    strAuthno = dt.Rows[0]["HSRPRecord_AuthorizationNo"].ToString();
                    StrAuthdate = dt.Rows[0]["RegistrationDate"].ToString();
                   ...
                   // StrVehicleType = StrNICVehicleType;
                    string strquery = "select top 1 upper(ourValue) as ourValue from  [dbo].[Mapping_Vahan_HSRP] where vahanvalue ='" + StrVehicleClassType + "'";
                    string StrVehicleClass = Utils.getDataSingleValue(strquery, ConnectionString, "ourValue");

                    StrTransactonType = dt.Rows[0]["HSRP_FLAG"].ToString();
                    string StrNICVehicleType = dt.Rows[0]["NICvehicletype"].ToString();
                   // StrMobileNo = dt.Rows[0]["MobileNo"].ToString();
                    StrManufacturarName = dt.Rows[0]["MAKER"].ToString();
                    StrModelName = dt.Rows[0]["MAKER_MODEL"].ToString();
                    ...
                    SQLString2 = "select dbo.hsrpplateamt ('" + HSRPStateID + "','" + StrVehicleClass + "','" + StrNICVehicleType + "','" + StrTransactonType + "') as Amount";
                    DataTable dt1 = Utils.GetDataTable(SQLString2, ConnectionString);
                    lblAmount.Text = dt1.Rows[0]["Amount"].ToString();

                    if ((StrNICVehicleType == "MCV/HCV/TRAILERS") || (StrNICVehicleType == "LMV(CLASS)") || (StrNICVehicleType == "LMV"))
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtMobileNo.Text.Trim() == "" || txtMobileNo.Text.Trim() == null)
                {
                    lblErrMess.Visible = true;
                    lblErrMess.Text = "";
                ...
                else
                {
                    ...
                    con.Open();
                    SqlCommand cmd = new SqlCommand("New_InsertDataHP", con);
                    ...
                    if (message == "Y")
                    {
                        ...
                        string smsmessage = "Cash Rs." + lblAmount.Text.Trim() + " received against HSRP " + txtRegNo.Text.Trim() + " on " + DateTime.Now.ToString("dd-MM-yyyy") + " receipt number " + cashrc + ". Transport Dept HP.";// is " + affdate.ToString("dd-MM-yyyy") + ". at your HSRP affixation center. Team HSRP";
                        ...
                        sr.Close();
                        myRespose.Close();
                        string qry = "insert into SMSlog_HP([hsrp_stateid],[MobileNo],[REGN_NO],[AUTH_NO],[HSRP_FLAG],[CashReceiptSmsText],[CashReceiptSMSDateTime],[CashReceiptSMSServerResponseID]) values ('"+HSRPStateID+"','" + MobileNo + "','" + txtRegNo.Text + "','" + lblAuthNo.Text.Trim() + "','" + lblTransactionType.Text.Trim() + "','" + smsmessage + "',getdate(),'" + result.ToString() + "') ";
                        int j = Utils.ExecNonQuery(qry, ConnectionString);
        ...
        public void EpsionPrint()
        {
            ...
            string SQLString = "select hsrprecordID,CashReceiptNo, HSRPRecord_CreationDate,HSRP_StateID,RTOLocationID,HSRPRecord_AuthorizationNo,HSRPRecord_AuthorizationDate,VehicleRegNo,convert(varchar(12),OwnerName)as OwnerName,MobileNo,VehicleClass,OrderType,StickerMandatory,IsVIP,NetAmount,VehicleType from hsrprecords where  vehicleRegNo ='" + txtRegNo.Text + "'";
