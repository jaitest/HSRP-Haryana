...
{
    public partial class DownloadAgentProductionWithDate : System.Web.UI.Page
    {
        ...
        protected void btnDownloadRecords_Click(object sender, EventArgs e)
        {
            ...
            if (dropdownDuplicateFIle.SelectedItem.Text == "--Select File Name--")
            {
                obj.FillHSRPProduction(AuthorizationDate, ToDate, DropDownListStateName.SelectedValue, dropDownListClient.SelectedValue, type, dropdownDuplicateFIle.SelectedValue, ref dt);
            }
            else
            {
                type = "4";
                string FileDateDate = dropdownDuplicateFIle.SelectedValue; 
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    PdfPCell cell1209329 = new PdfPCell(new Phrase(dt.Rows[i]["netAmount"].ToString(), new iTextSharp.text.Font(bfTimes, 6f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                    cell1209329.Colspan = 1;
                    cell1209329.MinimumHeight = 25f;
