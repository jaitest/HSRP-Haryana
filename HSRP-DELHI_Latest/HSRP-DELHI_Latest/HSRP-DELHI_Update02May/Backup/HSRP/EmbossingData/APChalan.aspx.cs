...
namespace HSRP.EmbossingData
{
    public partial class APChalan : System.Web.UI.Page
    {
        ...
        private void ShowGrid( string strFromdate,string strToDate)
        {
            ...
            {
                //string SQLString = "select Row_Number() over(order by orderstatus) as SNo, hsrprecordid,HSRPRecord_AuthorizationNo,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
                //    "from hsrprecords where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" +
                //    dropDownListClient.SelectedValue + "' and orderstatus ='Embossing Done' and hsrprecord_CreationDate between '" + strFromdate + "' and '" + strToDate + "' and Invoice_Flag='N' and hsrp_rear_lasercode is not null and hsrp_Front_lasercode is not null " +
                ...
                //string SQLString = "select Row_Number() over(order by orderstatus) as SNo, hsrprecordid,HSRPRecord_AuthorizationNo,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
                //   "from hsrprecords where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" +
                //   dropDownListClient.SelectedValue + "' and orderstatus ='Embossing Done' and hsrprecord_CreationDate between '" + strFromdate + "' and '" + strToDate + "'" +
                //   "order by VehicleClass,VehicleType,VehicleRegNo";
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                      strGetInvoiceNo = "select (isnull(prefixtext,'')+right('00000'+ convert(varchar,lastno+1),5)) from [hsrpstate] " +
                                               "where hsrp_stateid= '" + HSRPStateID + "' and prefixfor='Cash Receipt No' ";
