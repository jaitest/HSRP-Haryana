...
{
    public partial class MachineEmbossingReport1 : System.Web.UI.Page
    {
        ...
        protected void btnExportToExcel_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               // row.Cells.Add(new WorksheetCell("MachineID : " + dropDownMachineID.SelectedItem.ToString(), "HeaderStyle2"));
                ...
                {
                    //int.TryParse(DropDownListStateName.SelectedValue, out intHSRPStateID);
                    SQLString = "select a.machineid,b.creationdate,b.counter from CounterMachineConfig a inner join CounterTable as b on a.machineid =b.mid where b.command in ('44','45') and a.hsrpstateid='" + DropDownListStateName.SelectedValue.ToString() + "' and a.MachineID ='" + dropDownMachineID.SelectedItem.ToString() + "' and b.creationdate between '" + FromDate.ToString() + "' and '" + ToDate.ToString() + "'";
                }
                else
                {
                    if (CheckBox1.Checked ==true )
                        SQLString = "select a.machineid,b.creationdate,b.counter from CounterMachineConfig a inner join CounterTable as b on a.machineid =b.mid where  b.command in ('44','45') and a.hsrpstateid='" + DropDownListStateName.SelectedValue.ToString() + "' and b.creationdate between '" + FromDate.ToString() + "' and '" + ToDate.ToString() + "' order by  b.mid, b.creationdate";
                    else
