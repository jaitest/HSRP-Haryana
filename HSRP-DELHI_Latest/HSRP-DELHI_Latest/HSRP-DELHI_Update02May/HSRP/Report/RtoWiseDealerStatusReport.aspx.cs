...
{
    public partial class RtoWiseDealerStatusReport : System.Web.UI.Page
    {
        ...
        private Boolean validate1()
        {
            ...
            String getvalue2 = string.Empty;

            getvalue = DropDownListStateName.SelectedItem.Text;
            getvalue1 = dropDownListClient.SelectedItem.Text;
            getvalue2 = ddlBothDealerHHT.SelectedItem.Text;
            if (getvalue == "--Select State--" || getvalue1 == "--Select Location--" || getvalue2 == "--Select Dealer--")
            {
                blnvalid = false;
        ...
        private void Filldropdowndealer()
        {
            ...
            if (UserType == 0)
            {
                SQLString = "select distinct dealerid,d.[NAME OF THE DEALER] from hsrprecords as h,delhi_dealermaster as d where h.dealerid=d.SNO AND addrecordby='dealer' and dealerid!='' and HSRP_StateID=" + HSRPStateID + " and rtolocationid='" + dropDownListClient.SelectedValue + "' and orderstatus in ('Embossing Done','closed') and hsrprecord_creationdate between '" + FromDate + "' and '" + ToDate + "'  group by dealerid, d.[NAME OF THE DEALER]   order by 2 ";
                Utils.PopulateDropDownList(ddlBothDealerHHT, SQLString.ToString(), CnnString, "--Select Dealer--");
        ...
        protected void ddlBothDealerHHT_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string dealerid = " SELECT SNO FROM DELHI_DEALERMASTER WHERE [NAME OF THE DEALER]='" + ddlBothDealerHHT.SelectedItem + "'";
            string i = Utils.getDataSingleValue(dealerid, CnnString, "SNO");

            string sql = "select [vehicleregno],[HSRPRecord_AuthorizationNo],[OrderType],[ChassisNo],[EngineNo],[VehicleType],hsrp_front_lasercode,hsrp_rear_lasercode from hsrprecords where addrecordby='dealer' and dealerid!='' and HSRP_StateID=" + HSRPStateID + " and rtolocationid='" + dropDownListClient.SelectedValue + "' and orderstatus in ('Embossing Done','closed') and dealerid='" + i + "' and hsrprecord_creationdate between '" + FromDate + "' and '" + ToDate + "'";
            dt = Utils.GetDataTable(sql, CnnString);
