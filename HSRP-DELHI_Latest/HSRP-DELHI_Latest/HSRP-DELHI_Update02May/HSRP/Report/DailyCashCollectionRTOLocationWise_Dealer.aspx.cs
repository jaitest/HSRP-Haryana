...
{
    public partial class DailyCashCollectionRTOLocationWise_Dealer : System.Web.UI.Page
    {
        ...
        #region Dealer Bound
        private void Filldropdowndealer(String StringOrderDate, String StringAuthDate)
        {
            lbldealer.Visible = true;
            if (UserType.Equals(0))
            {
                int.TryParse(dropDownListClient.SelectedValue, out intRTOLocationID);
                //string SQLString1 = "select distinct h.dealerid,a.[NAME OF THE DEALER]  from delhi_dealermaster a,hsrprecords as h where a.SNO=h.dealerid  and h.rtolocationid='" + dropDownListClient.SelectedValue.ToString() + "'  and  h.HSRP_StateID=2 ";
                string SQLString1 = "Select distinct [Name of the Dealer],dealerid FROM HSRPRecords h,delhi_dealermaster d  where HSRP_StateID='2' and RTOLocationID='" + dropDownListClient.SelectedValue.ToString() + "' and dealerid is not null and dealerid!=0 and dealerid!=''  and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and NetAmount <> 0 and dealerid=sno order by [Name of the Dealer]";
                DataTable dt = new DataTable();
                dt = Utils.GetDataTable(SQLString1, CnnString);
                if (dt.Rows.Count > 0)
                {
                    Utils.PopulateDropDownList(ddlBothDealerHHT, SQLString1.ToString(), CnnString, "Select Dealer");
                    LabelError.Text = "Please Select Dealer";
                }
                else
                {

                    LabelError.Text = "No Record Found for Selected Date";
                }
            }
            else
            {
        ...
        private void ExportRecordExcel(Workbook book, DataTable dt, string RtoName = null, string RtoCode = null)
        {
            ...
            try
            {
                ...
                row.Cells.Add(new WorksheetCell(ddlBothDealerHHT.SelectedValue.ToString().Trim(), "HeaderStyle2"));
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            String StringOrderDate = OrderDate.SelectedDate.ToString("yyyy/MM/dd") + " 00:00:00";
            String StringAuthDate = HSRPAuthDate.SelectedDate.ToString("yyyy/MM/dd") + " 23:59:59";
            Filldropdowndealer(StringOrderDate, StringAuthDate);
        ...
        {
            if (ddlBothDealerHHT.SelectedValue == "Select Dealer")
            {
                LabelError.Text = "Please Select Dealer.";
                GridView1.DataSource = "";
                GridView1.DataBind();
            ...
            else
            {
                ...
                try
                {
                    ...
                    string SQLString1 = "select distinct dealerid  from delhi_dealermaster a,hsrprecords as h where a.SNO=h.dealerid and H.hsrprecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and  h.HSRP_StateID=2 and a.[NAME OF THE DEALER]='" + DealerName + "' ";
                ...
                if (ddlVehicleReference.SelectedItem.Text == "Both" && DropDownListVehicleModel.SelectedItem.Text == "All")
                {
                    //SQLString = "SELECT  HSRP_StateID,convert(varchar(15),HSRPRecord_CreationDate,103) as CashReceiptDateTime,(select (userfirstname+' '+UserLastName) as Name from users u where h.createdby=u.userid) as username,OrderStatus,case when orderstatus='Closed' and (orderembossingdate is null or orderembossingdate='') then convert(varchar(10),dateadd(dd,2,orderdate),103) else convert(varchar(10),orderembossingdate,103)end as embdate, CONVERT(varchar(20), HsrpRecord_AuthorizationDate, 103) AS HsrpRecord_AuthorizationDate,  CONVERT(varchar(20), HsrpRecord_creationdate, 103) AS NewOrderDate,CONVERT(varchar(20), OrderEmbossingDate, 103) AS OrderEmbossingDate,CONVERT(varchar(20), InvoiceDateTime, 103) AS InvoiceDateTime,  HSRPRecord_AuthorizationNo,CashReceiptNo, HSRPRecordID, VehicleRegNo, OwnerName, VehicleType, VehicleClass,CONVERT(numeric,round( RoundOff_NetAmount,0)) as NetAmount,OrderDate,EngineNo,ChassisNo,MobileNo,convert(varchar(10),ordercloseddate,103) as AffixDate FROM HSRPRecords h  where  HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "' and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by OrderDate";
                    SQLString = "select ROW_NUMBER() Over (Order by Hsrprecord_Creationdate) As [SNo],convert(varchar(20),Hsrprecord_Creationdate,103) as OrderDate,VehicleRegNo as [VehicleRegNo],OwnerName as OwnersName,VehicleType as VechicleType,VehicleClass as VehicleClass ,Case when orderstatus='Closed' then 'Affixed' else '' end as AffixationStatus, CONVERT(numeric,round( RoundOff_NetAmount,0)) as Amount FROM HSRPRecords h  where  HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "' and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by Hsrprecord_Creationdate";
                }
                else if (ddlVehicleReference.SelectedItem.Text == "Both")
                {
                    //SQLString = "SELECT  HSRP_StateID,convert(varchar(15),HSRPRecord_CreationDate,103) as CashReceiptDateTime,(select (userfirstname+' '+UserLastName) as Name from users u where h.createdby=u.userid) as username,OrderStatus,case when orderstatus='Closed' and (orderembossingdate is null or orderembossingdate='') then convert(varchar(10),dateadd(dd,2,orderdate),103) else convert(varchar(10),orderembossingdate,103)end as embdate, CONVERT(varchar(20), HsrpRecord_AuthorizationDate, 103) AS HsrpRecord_AuthorizationDate,  CONVERT(varchar(20), HsrpRecord_creationdate, 103) AS NewOrderDate,CONVERT(varchar(20), OrderEmbossingDate, 103) AS OrderEmbossingDate,CONVERT(varchar(20), InvoiceDateTime, 103) AS InvoiceDateTime,  HSRPRecord_AuthorizationNo,CashReceiptNo, HSRPRecordID, VehicleRegNo, OwnerName, VehicleType, VehicleClass,CONVERT(numeric,round( RoundOff_NetAmount,0)) as NetAmount,OrderDate,EngineNo,ChassisNo,MobileNo,convert(varchar(10),ordercloseddate,103) as AffixDate FROM HSRPRecords h where HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "' and VehicleType='" + DropDownListVehicleModel.SelectedItem.ToString() + "'  and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by OrderDate";
                    SQLString = "select ROW_NUMBER() Over (Order by Hsrprecord_Creationdate) As [SNo],convert(varchar(20),Hsrprecord_Creationdate,103) as OrderDate,VehicleRegNo as [VehicleRegNo],OwnerName as OwnersName,VehicleType as VechicleType,VehicleClass as VehicleClass ,Case when orderstatus='Closed' then 'Affixed' else '' end as AffixationStatus, CONVERT(numeric,round( RoundOff_NetAmount,0)) as Amount FROM HSRPRecords h where HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "' and VehicleType='" + DropDownListVehicleModel.SelectedItem.ToString() + "'  and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by Hsrprecord_Creationdate";
                }
                else if (ddlVehicleReference.SelectedItem.Text == "New" || ddlVehicleReference.SelectedItem.Text == "Old" && DropDownListVehicleModel.SelectedItem.Text == "All")
                {
                    //SQLString = "SELECT  HSRP_StateID,convert(varchar(15),HSRPRecord_CreationDate,103) as CashReceiptDateTime,(select (userfirstname+' '+UserLastName) as Name from users u where h.createdby=u.userid) as username,OrderStatus,case when orderstatus='Closed' and (orderembossingdate is null or orderembossingdate='') then convert(varchar(10),dateadd(dd,2,orderdate),103) else convert(varchar(10),orderembossingdate,103)end as embdate, CONVERT(varchar(20), HsrpRecord_AuthorizationDate, 103) AS HsrpRecord_AuthorizationDate,  CONVERT(varchar(20), HsrpRecord_creationdate, 103) AS NewOrderDate,CONVERT(varchar(20), OrderEmbossingDate, 103) AS OrderEmbossingDate,CONVERT(varchar(20), InvoiceDateTime, 103) AS InvoiceDateTime,  HSRPRecord_AuthorizationNo,CashReceiptNo, HSRPRecordID, VehicleRegNo, OwnerName, VehicleType, VehicleClass,CONVERT(numeric,round( RoundOff_NetAmount,0)) as NetAmount,OrderDate,EngineNo,ChassisNo,MobileNo,convert(varchar(10),ordercloseddate,103) as AffixDate FROM HSRPRecords h where HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "'  and vehicleref='" + ddlVehicleReference.SelectedItem.ToString() + "' and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by OrderDate";
                    SQLString = "select ROW_NUMBER() Over (Order by Hsrprecord_Creationdate) As [SNo],convert(varchar(20),Hsrprecord_Creationdate,103) as OrderDate,VehicleRegNo as [VehicleRegNo],OwnerName as OwnersName,VehicleType as VechicleType,VehicleClass as VehicleClass ,Case when orderstatus='Closed' then 'Affixed' else '' end as AffixationStatus, CONVERT(numeric,round( RoundOff_NetAmount,0)) as Amount FROM HSRPRecords h where HSRP_StateID='" + DropDownListStateName.SelectedValue + "' and RTOLocationID='" + dropDownListClient.SelectedValue + "'  and vehicleref='" + ddlVehicleReference.SelectedItem.ToString() + "' and HsrpRecord_creationdate between '" + StringOrderDate + "' and '" + StringAuthDate + "' and h.dealerid='" + dealerid + "' and NetAmount <> 0 order by Hsrprecord_Creationdate";
                }
                else
                {
