    ...
    public partial class HaryanaDataSync : System.Web.UI.Page
    {
        ...
        private void Export(string strReportType, Workbook book, int iActiveSheet, string strProcName)
        {
            try
            {
                ...
                #region Fetch Data
                SQLString = "select RTOLocationname,max(registrationdate) as RegistrationDate,DATEDIFF(day,max(registrationdate),getdate()) as SyncDelay from HSRPRecordsStaggingArea a,rtolocation b where a.RTOLocationID=b.RTOLocationID and b.HSRP_StateID='" + HSRPStateID + "' group by RTOLocationName order by 3";
                ...
                WorksheetCell cell = row.Cells.Add("Collection Report");
