...
{
    public partial class DownloadDelhiProductionWithDate : System.Web.UI.Page
    {
        ...
        private void FilldropDownListClient()
        {
            if (UserType == "0")
            {
                ...
                SQLString = "select distinct NAVEMBID  from RTOLocation Where HSRP_StateID=" + intHSRPStateID + " and ActiveStatus!='N'";
            ...
            else
            {
               ...
               // SQLString = "SELECT  distinct (a.RTOLocationName) as RTOLocationName, (a.RTOLocationCode+', ') as RTOCode, a.RTOLocationID FROM UserRTOLocationMapping INNER JOIN RTOLocation a ON UserRTOLocationMapping.RTOLocationID = a.RTOLocationID where UserRTOLocationMapping.UserID='" + UserID + "' order by RTOLocationName ";
                ...
                SQLString = "select distinct NAVEMBID  from RTOLocation Where HSRP_StateID='" + HSRPStateID + "' and ActiveStatus!='N'";
        ...
        protected void btnDownloadRecords_Click(object sender, EventArgs e)
        {
            ...
            //if (dropDownListClient.SelectedItem.ToString() == "--Select Embossing Center--")
            //{
            ...
            BAL obj = new BAL();
             ...
             string date = OrderDate.SelectedDate.ToString("dd-MM-yyyy", CultureInfo.InvariantCulture);
            ...
            //string MonthdateTO = MonTo.Replace("00", "0").Replace("01", "1");
             String ReportDateFrom = date;// StringAuthDate[1] + "/" + MonthdateTO + "/" + StringAuthDate[2].Split(' ')[0];

             String From = date;// StringAuthDate[0] + "/" + StringAuthDate[1] + "/" + StringAuthDate[2].Split(' ')[0];

            string AuthorizationDate = OrderDate.SelectedDate.ToString();// From + " 00:00:00";
           ...
           // string datestring = DateTime.Now.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            ...
            String FromDate = date;// StringOrderDate[1] + "-" + Monthdate + "-" + StringOrderDate[2].Split(' ')[0];
            String ReportDateTo = date;//StringOrderDate[1] + "/" + Monthdate + "/" + StringOrderDate[2].Split(' ')[0];
           ...
           // StrSqlQuery = "select NAVEMBID from rtolocation where hsrp_stateid='" + strStateID + "' and RtolocationId='" + RTOLocationID + "'";
            ...
            strEmbId = dropDownListClient.SelectedItem.ToString();
            



            string strSel = "select isnull(max(right(ProductionSheetRunningNo,7)),0000000) from EmbossingCenters where State_Id='" +Convert.ToInt32( DropDownListStateName.SelectedValue) + "' and Emb_Center_Id='" + strEmbId + "'";
            ...
            {
                strVehicleType =  " and a.vehicletype in ('MOTOR CYCLE','SCOOTER') ";
            }
            if (ddVehicleType == "THREE WHEELER")
            {
                strVehicleType = " and a.vehicletype in ('THREE WHEELER') ";
            }
            if (ddVehicleType == "FOUR WHEELER")
            {
                strVehicleType = " and a.vehicletype in ('LMV','LMV(CLASS)','MCV/HCV/TRAILERS','TRACTOR') ";
            }
            ...
            else
            {
              ...
              DataTable  dtdealerid = Utils.GetDataTable(DealerId, CnnString);
              for (int i = 0; dtdealerid.Rows.Count > i; i++)
              {
                  ...
                  if (Dname == DealerName)
                  {
                      ...
                      strSQL.Append("Select a.hsrprecordID,a.roundoff_netamount,a.OrderStatus, a.HSRPRecord_AuthorizationNo,convert(varchar, OrderClosedDate, 105) as OrderClosedDate,CONVERT(varchar(20),orderdate ,103) AS OrderBookDate,CONVERT(varchar(20),OrderEmbossingDate ,103) AS OrderEmbossingDate, a.dealerid as ID,left(a.OwnerName,19) as OwnerName, b.plate_netamount as ExShowRoomPrice, a.MobileNo,(select  AffixCenterDesc from AffixationCenters where Affix_id= a.affix_id ) as AffixCenterDesc,a.HSRPRecordID, CONVERT(varchar(20), a.HSRPRecord_AuthorizationDate,103) AS OrderDateAuth, a.OrderDate, a.EngineNo, a.ChassisNo,(select rtolocationname from rtolocation where  rtolocationid =a.rtolocationid) as RTOLocationName,  a.VehicleRegNo, case a.VehicleType when 'MCV/HCV/TRAILERS' then 'Trailers' when 'THREE WHEELER' then 'T.Whe.' when 'SCOOTER' then 'SCOO' when 'TRACTOR' then 'TRAC' when 'LMV(CLASS)'then 'L.CL' when 'LMV' then 'LMV'  when 'MOTOR CYCLE' then 'MO.C' end as VehicleType, case a.VehicleClass when 'Transport' then 'T' else 'N.T.' end as VehicleClass,a.HSRP_StateID, (select HSRPStateName from hsrpstate where HSRP_StateID=a.HSRP_StateID)  as HSRPStateName,(select replace(ProductCode,'MM-','') from Product where productid= a.RearPlateSize) AS RearProductCode,(select replace(ProductCode,'MM-','') from Product where productid= a.FrontPlateSize) AS FrontProductCode,a.FrontPlateSize, a.HSRP_Front_LaserCode, a.HSRP_Rear_LaserCode, a.RearPlateSize FROM HSRPRecords AS a   ,vendor_hsrprecords b  where   a.vehicleregno=b.vehicleregno and  a.hsrp_StateID='2' and a.NAVEMBID='" + RTOLocationID + "' and a.DealerId='" + Did + "'  and convert(date,a.hsrprecord_CreationDate) = convert(date,'" + AuthorizationDate + "') and ([HSRP_Front_LaserCode] is not null or [HSRP_Rear_LaserCode] is not null)  and ([HSRP_Front_LaserCode] !='' or [HSRP_Rear_LaserCode] !='')  ");
                  }
              }
            }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    //Opens the document:
                        ...
                        PdfPCell cell12092 = new PdfPCell(new Phrase("Dealer Name : " + ddlBothDealerHHT.SelectedItem.ToString(), new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                    ...
                    else if (HSRPStateID == "1" || HSRPStateID == "2" || HSRPStateID == "3" || HSRPStateID == "6" || HSRPStateID == "9" || HSRPStateID == "11" || (HSRPStateID == "4" && ddlBothDealerHHT.SelectedItem.Text.Equals("Both") || ddlBothDealerHHT.SelectedItem.Text.Equals("Other")))
                    {
                        ...
                        PdfPCell cell120933 = new PdfPCell(new Phrase("RTOLocationName", new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                        cell120933.Colspan = 1;
                        cell120933.BorderWidthLeft = 0f;
                        cell120933.BorderWidthRight = 0.8f;
                        cell120933.BorderWidthTop = 0.8f;
                    ...
                    if (strStateID == "1")
                    {
                    ...
                    else
                     {
                        ...
                        //table.AddCell(cell120939);
                         ...
                         PdfPCell cell120939 = new PdfPCell(new Phrase("ExShowRoomPrice", new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                        cell120939.Colspan = 1;
                        cell120939.BorderWidthLeft = 0f;
                        cell120939.BorderWidthRight = 0.8f;
                        cell120939.BorderWidthTop = 0.8f;
                    ...
                    for (int i = 0; i <= dt.Rows.Count - 1; i++)
                    {
                        ...
                        if (total == 44)
                        {
                            ...
                            PdfPCell cell120934 = new PdfPCell(new Phrase("Dealer Name : "+ ddlBothDealerHHT.SelectedItem.ToString(), new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                            ...
                            else
                            {
                                ...
                                PdfPCell cell120933a = new PdfPCell(new Phrase("RTOLocationName", new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                                cell120933a.Colspan = 1;
                                cell120933a.BorderWidthLeft = 0f;
                                cell120933a.BorderWidthRight = 0.8f;
                                cell120933a.BorderWidthTop = 0.8f;
                            ...
                            else
                            {
                                ...
                                PdfPCell cell120934a = new PdfPCell(new Phrase("ExShowRoomPrice", new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                                cell120934a.Colspan = 1;
                                cell120934a.BorderWidthLeft = 0f;
                                cell120934a.BorderWidthRight = 0.8f;
                                cell120934a.BorderWidthTop = 0.8f;
                        ...
                        else
                        {
                           // RTOLocationName
                            ...
                            PdfPCell cell1209315 = new PdfPCell(new Phrase(dt.Rows[i]["RTOLocationName"].ToString() , new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                            cell1209315.Colspan = 1;
                            cell1209315.MinimumHeight = 0f;//25
                       ...
                       // string Exs
                        ...
                        else
                        {
                            ...
                            PdfPCell cell1209329 = new PdfPCell(new Phrase(dt.Rows[i]["ExShowRoomPrice"].ToString(), new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                            cell1209329.Colspan = 1;
                            cell1209329.MinimumHeight = 0f;//25
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i <= dt.Rows.Count - 1; i++)
                            {
                                ...
                                sb.Append("update hsrprecords set sendtoProductionStatus='Y', NAVPDFFlag='1', PdfRunningNo='" + strRunningNo + "', PdfDownloadDate=GetDate(), pdfFileName='" + filename + "', PDFDownloadUserID='" + strUserID + "' where hsrprecordID='" + dt.Rows[i]["hsrprecordID"].ToString() + "';");
