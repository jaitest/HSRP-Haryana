...
using System.IO;
using HSRP.APWebrefrence;
using System.Net;

namespace HSRP.Master
{
    public partial class LaserQuickClosed_AP : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SQLString = "select hsrprecordid from hsrprecords where HSRP_StateID='" + HSRPStateID + "' and HSRPRecord_AuthorizationNo='" + lblauthno.Text.Trim().ToUpper() + "' and orderstatus ='Embossing Done' and RecievedAffixationStatus='Y' and orderdate>'1-jan-2015 00:00:00'";
                ...
                {
                    UserID = Session["UID"].ToString();
                    SQLString = "update hsrprecords set closeduserid = '" + UserID + "',orderstatus='Closed',orderclosedDate=GETDATE(),hsrp_rear_lasercode= '" + lblRearLaserPlate.Text + "',hsrp_front_lasercode='" + lblFrontLaserPlate.Text + "' where hsrprecordid ='" + hsrpid + "' ";
                ...
                string StrQuery = "update hsrprecords set APwebservicerespAffdate=getdate() where hsrp_stateid='9' and  Vehicleregno='" + textBoxVehicleRegNo.Text + "'";
                ...
                HSRP.APWebrefrence.HSRPAuthorizationService objAPAssign = new HSRP.APWebrefrence.HSRPAuthorizationService();
                // strEmbflag = objAPAssign.UpdateHSRPLaserCodes(textBoxVehicleRegNo.Text, textBoxFrontPlate.Text, textBoxRear.Text, serverdate);
                strEmbflag = objAPAssign.UpdateHSRPAffixation(lblauthno.Text, serverdate);
                ...
                else
                {
                    ...
                    if (obj.QuickAssignLaser_AP(int.Parse(HSRPStateID), FrontLaserPlate, RearLaserPlate, lblauthno.Text, int.Parse(UserID), hidvehicleclass.Value, hidvehicletype.Value, hidordertype.Value, ref Isexists))
                    {
                        //if (Isexists.Equals(0))
                        {
                            ...
                            Utils.ExecNonQuery(Query, CnnString);
                            string serverdate = System.DateTime.Now.ToString("dd/MM/yyyy");
                            string strQuery = "update hsrprecords set APwebservicerespEmbdate=getdate(),OrderEmbossingDate=getdate(),OrderStatus='Embossing Done',EmbossingUserID='" + UserID + "' where hsrp_stateid=9 and vehicleregno='" + textBoxVehicleRegNo.Text + "' ";
                            Utils.ExecNonQuery(strQuery, CnnString);
                            ...
                            HSRP.APWebrefrence.HSRPAuthorizationService objAPAssign = new HSRP.APWebrefrence.HSRPAuthorizationService();
                            strEmbflag = objAPAssign.UpdateHSRPLaserCodes(lblauthno.Text, textBoxFrontPlate.Text, textBoxRear.Text, serverdate);
                            ...
                            {
                                mobileno = "select mobileno from hsrprecords where  hsrp_stateid=9 and hsrprecord_authorizationno ='" + lblauthno.Text.Trim().ToUpper() + "'";
                                ...
                                if (affdate1 != "1" && mobileno.Length == 10)
                                {
                                    ...
                                    string url = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-aphsrp&password=aphsrp&type=0&dlr=1&destination=" + MobileNo + "&source=APHSRP&message= " + smsmessage + "";
