...
using HSRP.APHSRPWebService;
using System.Xml;


namespace HSRP.Transaction
{
    public partial class CashReceiptDataEntryAP : System.Web.UI.Page
    {
        ...
        private void GetRateForVehicle(string HSRPStateID,string  StrVehicleType,string StrVehicleClassType, string StrTransactonType)
        {
            ...
            txtAmount.Text = "";
            SQLString = "select dbo.hsrpplateamt ('" + HSRPStateID + "','" + StrVehicleType + "','" + StrVehicleClassType + "','" + StrTransactonType + "') as Amount";
            DataTable dt = Utils.GetDataTable(SQLString, ConnectionString);
            txtAmount.Text = dt.Rows[0]["Amount"].ToString();

            SQLString = "select FrontPlateID,RearPlateID,StickerID from RegistrationPlateDetail  where hsrp_stateid ='" + HSRPStateID + "' and vehicletype='" + StrVehicleType + "' and vehicleclass='" + StrVehicleClassType + "' and ordertype='" + StrTransactonType + "'";
            ...
            if (dtPlateSize.Rows.Count > 0)
            {
                ...
                sqlstring1 = "select discountamount from Discount where hsrp_stateid ='" + HSRPStateID + "' and vehicletype='" + StrVehicleType+ "' and vehicleclass='" + StrVehicleClassType + "'";
                dt = Utils.GetDataTable(sqlstring1, ConnectionString);
                if (dt.Rows.Count > 0)
                {
        ...
        protected void btnDownloadReceipt_Click(object sender, EventArgs e)
        {
            ...
            if (dataSetFillHSRPDeliveryChallan.Rows.Count > 0)
            {
                ...
                PdfPCell cell312 = new PdfPCell(new Phrase("Original", new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                cell312.Colspan = 4;
                cell312.BorderColor = BaseColor.WHITE;
                cell312.HorizontalAlignment = 0; //0=Left, 1=Centre, 2=Right
                table1.AddCell(cell312);

                PdfPCell cell312a = new PdfPCell(new Phrase("Duplicate", new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                cell312a.Colspan = 4;
        ...
        protected void LinkButtonGO_Click(object sender, EventArgs e)
        {
            ...
            string strAuthNo = txtAuthorizationNo.Text.Trim();
            ...
            using (XmlTextReader reader = new XmlTextReader(stringReader))
            {
                while (reader.Read())
                {
                    ...
                    {
                        reader.Read();
                        string strquery = "select ourValue from  [dbo].[Mapping_Vahan_HSRP_ap] where rtovalue ='" + reader.Value.ToString() + "'";
                    ...
                    if (reader.Name.ToString() == "Trans_Type")
                    {
                        ...
                        //if (reader.Value.ToString() == "NB")
                        //{
                        //    StrTransactonType = "NEW BOTH PLATES";
                        //}
                        //else if (reader.Value.ToString() == "OB")
                        //{
                        //    StrTransactonType = "OLD BOTH PLATES";
                        //}
                        //else if (reader.Value.ToString() == "DB")
                        //{
                        //    StrTransactonType = "DAMAGED BOTH PLATES";
                        //}
                        //else if (reader.Value.ToString() == "DF")
                        //{
                        //    StrTransactonType = "DAMAGED FRONT PLATE";
                        //}
                        //else if (reader.Value.ToString() == "DR")
                        //{
                        //    StrTransactonType = "DAMAGED REAR PLATE";
                        //}
                        //else if (reader.Value.ToString() == "OS")
                        //{
                        //    StrTransactonType = "ONLY STICKER";
                        //}
                    ...
                    {
                        reader.Read();
                        StrRegistrationNo = reader.Value.ToString();
                        reader.Read();
                    }
               ...
               // SQLString = "select dbo.hsrpplateamt ('" + HSRPStateID + "','" + DropDownListVehicleModel.SelectedItem.ToString() + "','" + DropDownListVehicleClass.SelectedItem.ToString() + "','" + DropDownListOrderType.SelectedValue + "') as Amount";
                ...
                GetRateForVehicle(HSRPStateID, StrVehicleType,StrVehicleClassType, StrTransactonType);
            }
            ...
            if (AuthData.Length > 0)
            {
                ...
                txtAuthorizationNo.Text = StrAuthorizationNo;
                txtOwnerName.Text = StrOwnerName;
                txtAddress.Text = StrOwnerAddress;
                DropDownListVehicleModel.SelectedItem.Text = StrVehicleType;
                DropDownListOrderType.SelectedItem.Text = StrTransactonType;
                txtMobileNo.Text = StrMobileNo;
                DropDownListVehicleClass.SelectedItem.Text = StrVehicleClassType;
                txtVehicleRegNo.Text = StrRegistrationNo;
        ...
        // string Roundoff_NetAmount = string.Empty;
        ...
        // string ISFrontPlateSize = string.Empty;
        // string ISRearPlateSize = string.Empty;
        ...
        // string CheckMacStatus = string.Empty;

        public void saverecord()
        {


            // HSRPRecord_CreationDate = ;
            // HSRP_StateID = .Empty;
            // RTOLocationID = .Empty;
            // HSRPRecord_AuthorizationNo = .Empty;
            // HSRPRecord_AuthorizationDate = .Empty;
            // VehicleRegNo = .Empty;
            // OwnerName = .Empty;
            // ownerFatherName = .Empty;
            // Address1 = .Empty;
            ...
            // VehicleClass = .Empty;
            // OrderType = .Empty;
            // StickerMandatory = .Empty;
            // isVIP = .Empty;
            // NetAmount = .Empty;
            // Roundoff_NetAmount = .Empty;
            // VehicleType = .Empty;
            // OrderStatus = .Empty;
            // CashReceiptNo = .Empty;
            // ChassisNo = .Empty;
            ...
            // DealerCode = .Empty;
            // CreatedBy = .Empty;
            // SaveMacAddress = .Empty;
            // Addrecordby = .Empty;
            // ISFrontPlateSize = .Empty;
            // ISRearPlateSize = .Empty;
            // FrontPlateSize = .Empty;
            // RearPlateSize = .Empty;
            // Reference = .Empty;
            // ManufacturerModel = .Empty;
            // vehicleref = .Empty;
            // ManufacturerName = .Empty;
            // CheckMacStatus = .Empty;

            //   APDataCashCollection(HSRPRecord_CreationDate,  HSRP_StateID,  RTOLocationID,  HSRPRecord_AuthorizationNo,  HSRPRecord_AuthorizationDate,  VehicleRegNo,  OwnerName,  ownerFatherName,  Address1,  MobileNo,  VehicleClass,  OrderType,  StickerMandatory,  isVIP,  NetAmount,  Roundoff_NetAmount,  VehicleType,  OrderStatus,  CashReceiptNo,  ChassisNo,  EngineNo,  DealerCode,  CreatedBy,  SaveMacAddress,  Addrecordby,  ISFrontPlateSize,  ISRearPlateSize,  FrontPlateSize,  RearPlateSize,  Reference,  ManufacturerModel,  vehicleref,  ManufacturerName, CheckMacStatus)
            //}
