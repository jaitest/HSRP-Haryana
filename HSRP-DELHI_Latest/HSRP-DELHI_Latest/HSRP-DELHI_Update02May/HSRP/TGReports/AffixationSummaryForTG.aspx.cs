...
{
    public partial class AffixationSummaryForTG : System.Web.UI.Page
{
    int UserType1;
    ...
    protected void btnPDF_Click(object sender, EventArgs e)
    {
        ...
        strQuery = "select (select rtolocationname fro-m rtolocation where rtolocationid=a.rtolocationid) locationname,SUM(CASE WHEN (a.VehicleType='SCOOTER' or a.VehicleType='MOTOR CYCLE')  THEN 1 ELSE 0 END ) AS [2WCount],SUM(CASE WHEN (a.VehicleType = 'Three Wheeler') THEN 1 ELSE 0 END ) AS [3WCount],SUM(CASE WHEN (a.VehicleType = 'Tractor' )  THEN 1 ELSE 0 END ) AS [TractorCount],SUM(CASE WHEN ((a.VehicleType = 'LMV' or VehicleType='LMV(Class)') and VehicleClass ='Non-Transport')   THEN 1 ELSE 0 END ) AS [4WCount],SUM(CASE WHEN ((a.VehicleType = 'LMV' or VehicleType='LMV(Class)') and VehicleClass ='Transport')  THEN 1 ELSE 0 END ) AS [4Wtcount],SUM(CASE WHEN (a.Vehicletype ='MCV/HCV/TRAILERS')  THEN 1 ELSE 0 END ) AS [HeavyCount] from hsrprecords a  where  HSRP_StateID=11 and a.ordercloseddate between '"+ ReportDate1 +"' and '"+  ReportDate2 +"' and netamount >0 group by a.rtolocationid order by (select rtolocationname from rtolocation where rtolocationid=a.rtolocationid)";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            //GenerateCell(table, 10, 1, 1, 1, 1, 1, 0, "Telangana", 30, 0);
            string cname = Utils.getDataSingleValue("select CompanyName from HSRPState where hsrp_stateid=11", CnnString1, "CompanyName");
            string add = Utils.getDataSingleValue("select Address1 from HSRPState where hsrp_stateid=11", CnnString1, "Address1");
