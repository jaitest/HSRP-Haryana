...
{
    public partial class ReprintInvoiceDealer : System.Web.UI.Page
    {
        ...
        private DataTable GetRecords(string strRecordId)
        {
            ...
            if (!dropdownDuplicateFIle.SelectedItem.Text.Equals("--Select Challan No--"))
            {
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    string SQLString1 = "select distinct dealerid  from delhi_dealermaster a,hsrprecords as h where a.SNO=h.dealerid and H.hsrprecord_creationdate between '" + strFrmDateString + "' and '" + strToDateString + "' and  h.HSRP_StateID=2 and a.[NAME OF THE DEALER]='" + DealerName + "' ";
                    DealerId = Utils.getDataSingleValue(SQLString1, CnnString, "dealerid");
                }
                catch
                {}
                SQLString = "select  Distinct ChallanNo   from hsrprecords as h  where  hsrp_StateID='" + DropDownListStateName.SelectedValue +
                       "' and DealerId='" + DealerId + "' and  ChallanNo is not null and ChallanDate between '" +
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
            ...
            {
                #region Validation
                if (string.IsNullOrEmpty(dropdownDuplicateFIle.Text))
                {
                    Response.Write("<script> alert('Please Select Challan No.')</script>");
                    return;
                }
                if (string.IsNullOrEmpty(ddlBothDealerHHT.Text))
                {
                    Response.Write("<script> alert('Please Select Dealer.')</script>");
                    return;
                }
                #endregion
                ...
                if (true)
                {
                    ...
                    GenerateCell(table, 3, 1, 1, 0, 0, 0, 0, "", 0, 0);
                    GenerateCell(table, 7, 0, 1, 0, 0, 0, 0, "", 0, 0);

                 


                    GenerateCell(table, 3, 1, 1, 1, 0, 0, 0, "", 0, 0);
                    GenerateCell(table, 7, 0, 1, 1, 0, 0, 0, "", 0, 0);
