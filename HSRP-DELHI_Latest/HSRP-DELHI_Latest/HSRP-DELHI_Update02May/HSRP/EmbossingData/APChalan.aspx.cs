...
namespace HSRP.EmbossingData
{
    public partial class APChalan : System.Web.UI.Page
    {
        ...
        private void FilldropDownListOrganization()
        {
            ...
            {
                SQLString = "select HSRPStateName,HSRP_StateID from HSRPState  where HSRP_StateID='2' and ActiveStatus='Y' Order by HSRPStateName";
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string strFrmDateString = OrderDate.SelectedDate.ToShortDateString() + " 00:00:00";
                string strToDateString = HSRPAuthDate.SelectedDate.ToShortDateString() + " 23:59:59";
                if (ddlemb.SelectedItem.Text == "--Select Embossing Center Name--")
                {
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SQlQuery = "select vehicletype+' (Damage Front)' +VehicleClass as DescriptionOfGoods,count(*) as qty,round((sum(EXCISEBASIC)/count(*)),3) amt,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess  from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and ordertype in ('DF') and HsrpRecordID in(" + strHsrpRecordId + ") group by vehicletype ,VehicleClass union   select vehicletype+' (Damage Rear)'+ VehicleClass,count(*) as qty,round((sum(EXCISEBASIC)/count(*)),2) amt,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in(" + strHsrpRecordId + ") and  ordertype in ('DR') group by vehicletype,VehicleClass  union select vehicletype+' (Both Plates)'+VehicleClass,(count(*)*2)/2 qty,round((sum(EXCISEBASIC)/count(*)),2) amt,round(sum(EXCISEBASIC),2)bamt,round(sum(Vat_Amount),2)Vatamt,round(sum(EXCISEAMT),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in(" + strHsrpRecordId + ") and ordertype in ('NB','OB','DB') group by vehicletype, VehicleClass";
