    ...
    public partial class Embossing : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            if (dropDownListClient.SelectedItem.Text != "--Select RTO Name--")
            {
                ...
                {
                    SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,HSRPRecord_AuthorizationNo from hsrprecords where hsrp_stateid ='" + HSRPStateID + "' and Hsrprecord_creationdate between '" + AuthorizationDate + "' and  '" + ToDate + "'  and dealerid='" + DealerId + "' and vehicletype='" + ddVehicleType + "'  and orderstatus ='New Order' and HSRP_Front_LaserCode is not null and HSRP_Rear_LaserCode is not null  order by VehicleClass,VehicleType,VehicleRegNo";
                    dt = Utils.GetDataTable(SQLString, CnnString);
                }
                else
                {
                    SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,HSRPRecord_AuthorizationNo from hsrprecords where hsrp_stateid ='" + HSRPStateID + "' and Hsrprecord_creationdate between '" + AuthorizationDate + "' and  '" + ToDate + "'  and dealerid='" + DealerId + "'  and orderstatus ='New Order' and HSRP_Front_LaserCode is not null and HSRP_Rear_LaserCode is not null  order by VehicleClass,VehicleType,VehicleRegNo";
                    dt = Utils.GetDataTable(SQLString, CnnString);
                }
                if (dt.Rows.Count > 0)
                {
