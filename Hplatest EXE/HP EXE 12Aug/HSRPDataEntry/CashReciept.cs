...
namespace HSRPDataEntryNew
{
    public partial class CashReciept : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                txtVehRegNo.Text = "";
                MessageBox.Show("Cash Collection Timing is  Between 9 AM TO 6 PM .");
                txtVehRegNo.Text = "";
                txtVehRegNo.Focus();
                return;
            ...
            if (txtVehRegNo.Text.Trim().ToString().Length<2)
            {
                MessageBox.Show("Please Insert Vehicle Registration No.");
                txtVehRegNo.Text = "";
                txtVehRegNo.Focus();
            ...
            string strCheck = strtxtVehRegNo.Substring(0, 2);
               
                if (strCheck.ToUpper() != "HP")
                {
                    MessageBox.Show("Vehicle Registration Not in Correct Format.");
                    txtVehRegNo.Text = "";
                    txtVehRegNo.Focus();
                    return;
                }
            ...
            var authdate = AuthDate.Value.ToString("MM/dd/yyyy");


            if ( Convert.ToDateTime(authdate) > Convert.ToDateTime(_dateString))
            {
                MessageBox.Show(" Authorization Date Not Valid.");        
            ...
            if ((string.IsNullOrEmpty(txtMobileNo.Text.Trim())) || (txtMobileNo.Text.Trim().Length < 10) ||  (txtMobileNo.Text.Trim().Length > 10))
            {
           ...
           string amount = objWebServiceHP.GetRateAndTaxForVehicle(HSRP_StateID, ddlVehType.Text, ddlVehicleClass.Text,orderType);
            ...
            string ChassisNo = txtChassis.Text.Trim();
            ...
            string issave = objWebServiceHP.CheckDuplicateEntry(HSRP_StateID, txtAuthorizationNo.Text, txtVehRegNo.Text, orderType);
        ...
        private void ddlOrderType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else    if (ddlVehicleClass.Text != "--Select Vehicle Class--" && ddlVehType.Text != "--Select Vehicle Type--" && ddlOrderType.Text != "--Select Order Type--")
            {
        ...
        private void search_veh(string flag,string vehicleNo)
        {
            ...
            if (dt1.Rows.Count > 0)
            {
                txtVehRegNo.Text = dt1.Rows[0]["REGN_NO"].ToString().Replace(" ","");
        ...
        private void CashReciept_Load(object sender, EventArgs e)
        {
            ...
            if (isConnected())
            {
                ...
                RTOSTA.Text = "--Select RTO STA--";
                ddlordertypeOLd.Text= "--Select Order Type--";
                ddlVehicleClass.Text = "--Select Vehicle Class--";
            }
            else
            {
