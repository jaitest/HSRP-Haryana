...
namespace HSRPWebServices
{
    ...
    public class HSRPService : System.Web.Services.WebService
    {
        ...
        public DataTable GetRecords(DataTable dt)
        {
            ...
            {
                str.Append("'" + dt.Rows[i]["HSRPRecord_AuthorizationNo"] + "',");
            ...
            string sqlText = "Select   HSRPRecord_AuthorizationNo, OrderEmbossingDate from hsrprecords  where hsrp_stateid='3' and HSRPRecord_AuthorizationNo in(" + str.ToString() + ")  and( hsrp_front_lasercode !=''  or isnull(hsrp_front_lasercode,'')!='' ) and ( isnull(HSRP_Rear_LaserCode,'')!='' or  hsrp_rear_lasercode !='')and( OrderEmbossingDate !='' or isnull(OrderEmbossingDate,'')!='') and orderstatus ='Embossing Done'  ";
        ...
        public string InsertDataCashCollection(string HSRP_StateID, string RTOLocationID, string HSRPRecord_AuthorizationNo, string HSRPRecord_AuthorizationDate, string VehicleRegNo, string OwnerName, string ownerFatherName, string Address1, string MobileNo, string VehicleClass, string OrderType, string StickerMandatory, string isVIP, string NetAmount, string Roundoff_NetAmount, string VehicleType, string OrderStatus, string CashReceiptNo, string ChassisNo, string EngineNo, string DealerCode, string CreatedBy, string SaveMacAddress, string Addrecordby, string ISFrontPlateSize, string ISRearPlateSize, string FrontPlateSize, string RearPlateSize, string Reference, string ManufacturerModel, string vehicleref, string ManufacturerName, string CounterNo, string Affixid)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string sqlText = "select COUNT(VehicleRegNo) as co from dbo.HSRPRecords  where LTRIM(RTRIM(hsrp_stateid)) =LTRIM(RTRIM('" +HSRP_StateID+ "')) and LTRIM(RTRIM(VehicleRegNo)) =LTRIM(RTRIM('" + VehicleRegNo.Trim().Replace(" ", "") + "')) and  LTRIM(RTRIM(OrderType))= LTRIM(RTRIM('" + OrderType + "')) and LTRIM(RTRIM( HSRPRecord_AuthorizationNo))= LTRIM(RTRIM('" + HSRPRecord_AuthorizationNo.Trim().Replace(" ", "") + "'))";
                        DataTable dt = new DataTable();
                        dt = utils.GetDataTable(sqlText, CnnString);
                        if (dt.Rows[0]["co"].ToString() != "0")
                        {
                          return Status = "Record is Duplicate" + "^" + VehicleRegNo + "^" + HSRPRecord_AuthorizationNo;
        ...
        public String CheckDuplicateEntry(string HSRPStateID, string AuthNo, string VehicleRegNo, string OrderType)
        {
            ...
            string sqlText = "select COUNT(VehicleRegNo) as co from dbo.HSRPRecords  where LTRIM(RTRIM(hsrp_stateid)) =LTRIM(RTRIM('" + HSRPStateID + "')) and LTRIM(RTRIM(VehicleRegNo)) =LTRIM(RTRIM('" + VehicleRegNo.Trim().Replace(" ", "") + "')) and  LTRIM(RTRIM(OrderType))= LTRIM(RTRIM('" + strOrderType + "')) and LTRIM(RTRIM( HSRPRecord_AuthorizationNo))= LTRIM(RTRIM('" + AuthNo.Trim().Replace(" ", "") + "'))";
