    ...
    public partial class SMSReport : System.Web.UI.Page
{
    ...
    private void Export(string strStateId, string strStateName)
    {
        try
        {
            ...
            book.Properties.Title = "AP-TS Summary";
          ...
          string  FromDate = OrderDate.SelectedDate.ToString("MM/dd/yy 00:00:00");
           string ToDate = HSRPAuthDate.SelectedDate.ToString("MM/dd/yy 23:59:59");
            if (DropDownListStateName.SelectedValue == "9")
            {
                SqlCommand cmd = new SqlCommand("[ApNewProcess]", con);
    ...
    {
        DataTable dt = new DataTable();
        string strstatid1 = DropDownListStateName.SelectedValue.ToString();
        string FromDate = OrderDate.SelectedDate.ToString("MM/dd/yy 00:00:00");
        string ToDate = HSRPAuthDate.SelectedDate.ToString("MM/dd/yy 23:59:59");
        if (DropDownListStateName.SelectedValue == "9")
        {
            SqlCommand cmd = new SqlCommand("ApNewProcess", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add(new SqlParameter("@stateid", strstatid1));
            cmd.Parameters.Add(new SqlParameter("@datefrom", FromDate));
            cmd.Parameters.Add(new SqlParameter("@dateto", ToDate));
            // cmd.Parameters.Add(new SqlParameter("@addrecordby", ddlBothDealerHHT.SelectedItem.Text));
