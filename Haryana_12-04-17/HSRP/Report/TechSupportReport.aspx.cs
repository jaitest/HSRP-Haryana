...
{
    public partial class TechSupportReport : System.Web.UI.Page
    {
        int UserType1;
        ...
        {
            SqlConnection con = new SqlConnection(SQLString1);
             con.Open();
            string str = "select  dealername,ownername,engineno,hsrprecord_authorizationNo,onlinepaymentid,state,vehicletype,vehicleclass,vehicleregno,entrydate,onlinepaymentstatus from hsrprecords";
          SqlCommand  cmdlist = new SqlCommand(str, con);
          SqlDataAdapter sqlda = new SqlDataAdapter(cmdlist);
           DataSet ds = new DataSet();
           sqlda.Fill(ds, "hsrprecords");
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ddlauthname.Items.Add(ds.Tables[0].Rows[i]["ownername"] + "---" + ds.Tables[0].Rows[i]["hsrprecord_authorizationNo"].ToString());
            }
        }
