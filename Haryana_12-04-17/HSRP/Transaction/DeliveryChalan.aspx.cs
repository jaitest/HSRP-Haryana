...
{
    public partial class DeliveryChalan : System.Web.UI.Page
    {
        ...
        {    
            if (dropDownListClient.SelectedItem.Text != "--Select RTO Name--")
            {
                SQLString = "select pdffilename, 'P_F '+ right(PdfFileName,14) +'    ( '+ Convert(nvarchar(50),count(PdfFileName))+' )' as PdfName from hsrprecords where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationid='" + dropDownListClient.SelectedValue + "' and PdffileName is not null group by pdffilename";
                Utils.PopulateDropDownList(dropDownListorderStatus, SQLString.ToString(), CnnString, "--Select File Name--");
            } 
        }
        ...
        protected void dropDownListorderStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            GridBindForGrid(); 
        }

        private void GridBindForGrid()
        {
            SQLString = "select ROW_NUMBER() OVER (ORDER BY vehicleRegNo) As SrNo, hsrpRecordID, vehicleRegNo,HSRP_Front_LaserCode,HSRP_Rear_LaserCode,(case when orderStatus ='New Order' then 'false' else 'true' end) as checkb  from hsrprecords where pdffilename ='" + dropDownListorderStatus.SelectedValue + "'  order by orderStatus";
            DataTable dts = Utils.GetDataTable(SQLString, CnnString);
            if (dts.Rows.Count > 0)
            {
                Grid1.DataSource = dts;
        ...
        //        PdfPCell cell122 = new PdfPCell(new Phrase("Rosmerta HSRP Ventures Pvt. Ltd.", new iTextSharp.text.Font(bfTimes, 12f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
        //        cell122.Colspan = 7;
        ...
        //        PdfPCell cell123 = new PdfPCell(new Phrase("Group Floor Kasra No 21/25, Main Road Kamalpur Majra ,Sant Nagar Village, Burari, delhi", new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
        //        cell123.Colspan = 7;
