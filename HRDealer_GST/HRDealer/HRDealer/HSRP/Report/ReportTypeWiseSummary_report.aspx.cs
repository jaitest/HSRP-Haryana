...
namespace HSRP.Report
{
    public partial class ReportTypeWiseSummary_report : System.Web.UI.Page
    {
        ...
        private void FilldropDownListStateName()
        {
            if (UserType1.Equals(0))
            {
                SQLString1 = "select HSRPStateName,HSRP_StateID from HSRPState where ActiveStatus='Y' and HSRP_StateID ='4' Order by HSRPStateName";
        ...
        private void Export(string strReportType, Workbook book, int iActiveSheet, string strProcName)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dt.Rows.Count; j++)
                    {                       
                     total += Convert.ToInt32(dt.Rows[j]["Total"].ToString());   
                    
                     }
                    int Index = iIndex++;
                    Index = Index +  dt.Rows.Count-1;
                    ...
                    row.Cells.Add(new WorksheetCell(total.ToString(), "HeaderStyle6"));
                   // WorksheetCell cell1 = row.Cells.Add(, "HeaderStyle6" );
                    cell.MergeAcross = 0; // Merge two cells together
                    cell.StyleID = "HeaderStyle6";
                    row = sheet.Table.Rows.Add();
