                ...
                else
                {
                    ...
                    if (Utils.ExecNonQuery(SQLString2, CnnString) > 0)
                    {
                        ...
                        DataTable dt1 = Utils.GetDataTable(Stringquery, CnnString);
                         if(dt1.Rows.Count>0)
                            {
                                string String = "Update DealerMaster Set userid = '" + dt1.Rows[0]["userid"].ToString() + "' where  dealerid ='" + dealerid + "' and  HSRP_StateID = '4' and  RTOLocationID = '" + DropdownRTOName.SelectedValue + "'";
