...
namespace HSRP.Transaction
{
    public partial class UploadDealerData : System.Web.UI.Page
    {
        ...
        //        sql1 = "select dealerName from DealerMaster where dealerid='" + dealerid + "'";
        //        Dealeardetail = Utils.getScalarValue(sql1, CnnString);
        //        lbldealername.Text = Dealeardetail.ToString();//int.Parse(DepositAmount);
        //        sql1 = string.Empty;
        //        Dealeardetail = string.Empty;
        //        sql1 = "select DealerID from DealerMaster where dealerid='" + dealerid + "'";
        //        Dealeardetail = Utils.getScalarValue(sql1, CnnString);
        //        lbldealercode.Text = Dealeardetail.ToString();

        //        sql1 = string.Empty;
        //        Dealeardetail = string.Empty;
        //        sql1 = "select City from DealerMaster where dealerid='" + dealerid + "'";
        //        Dealeardetail = Utils.getScalarValue(sql1, CnnString);
        //        lblarea.Text = Dealeardetail.ToString();
        ...
        //        DataTable dtCount = Utils.GetDataTable(SqlQuery, CnnString);
        //        string strVehicle = dtCount.Rows[0]["TodayVehicleCount"].ToString();
        //        string TodayCollection = dtCount.Rows[0]["TodayCollection"].ToString();
        ...
        //        if (string.IsNullOrEmpty(TodayCollection))
        //        {
        //            Session["Availableamt"] = strDepositAmount;
        //        }
        //        else
        //        {
        //            Session["Availableamt"] = Convert.ToInt32(strDepositAmount) - Convert.ToInt32(TodayCollection);
        //        }
                
        //        if (string.IsNullOrEmpty(dtCount.Rows[0]["DepositDate"].ToString()) )
        //        {
        ...
        //        lbllastDepositAmount.Text = strDepositAmount;
        //        lblAvailableAmount.Text = Convert.ToString(Convert.ToInt32(Session["Availableamt"].ToString()));
        ...
        //        else
        //        {
        //            lblCollection.Text = TodayCollection;
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        private void InsertionOfRecords(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                VClass = dr["VehicleClass"].ToString().Trim();
                              
                char[] vehicleclassarray = VClass.ToCharArray(0, 3);
                string ss = vehicleclassarray[0].ToString().Trim() + vehicleclassarray[1].ToString().Trim() + vehicleclassarray[2].ToString().Trim();
                ...
                string strVehicle = "select count(*) from hsrprecords where LTRIM(RTRIM(hsrp_stateid))= LTRIM(RTRIM('4')) and LTRIM(RTRIM(ChassisNo))= LTRIM(RTRIM('" + ChassisNo + "')) and LTRIM(RTRIM(OrderType))= LTRIM(RTRIM('" + OrderType.Trim() + "'))";
                ...
                if (Iresult > 0)
                {
                    ...
                    llbMSGError2.Text = "";
                    llbMSGError2.Text = "Duplicate Records Found.";
                ...
                else
                {
                    ...
                    SQLString2 = "select dealerid  from dealermaster where dealerid='" + DealerID+ "'";
                     ...
                     if (dt1.Rows.Count > 0)
                     {
                         ...
                         {
                             if (VehicleClass.ToUpper().Trim() == "NON-TRANSPORT" && VehicleType.Trim().ToUpper() == "SCOOTER" || VehicleType.Trim().ToUpper() == "MOTOR CYCLE" || VehicleType.Trim().ToUpper() == "TRACTOR")
                             {
                                 fixcharge = "118.00";
                             }

                             if (VehicleClass.Trim() == "NON-TRANSPORT" && VehicleType.Trim() == "L.M.V. (CAR)" || VehicleType.Trim().ToUpper() == "LMV" || VehicleType.Trim().ToUpper() == "LMV(CLASS)" || VehicleType.Trim() == "MCV/HCV/TRAILERS" )
                             {
                                 fixcharge = "295.00";
                             }
                             ...
                             if (VehicleType.Trim().ToUpper() == "THREE WHEELER")
                             {
                                 fixcharge = "118.00";
                             }
        ...
        private void ValidationCheckOnRecords(DataTable ExcelSheet)
        {
            ...
            for (int i = 0; i < ExcelSheet.Rows.Count; i++)
            {
                ...
                string vehicleclass = ExcelSheet.Rows[i]["VehicleClass"].ToString().Trim();
                int vehicleclassLength = vehicleclass.Length;

                if (vehicleclassLength<3)
                {
                    i = i + 2;
                    llbMSGError.Text = "Excel Sheet : Has <b>VEHICLECLASS</b> Field Wrong At Row : " + i;
                    FlagIsDirty = true;
                    return;
                }

                char[] vehicleclassarray = vehicleclass.ToCharArray(0, 3);
                string   vclass= vehicleclassarray[0].ToString().Trim()+vehicleclassarray[1].ToString().Trim()+vehicleclassarray[2].ToString().Trim();
                if (vclass.ToUpper() != "TRA")
                {  
                    if (vclass.ToUpper() != "NON")
                    {
                        i = i + 2;
                        llbMSGError.Text = "Excel Sheet : Has <b>VEHICLECLASS</b> Field Wrong At Row : " + i;
                        FlagIsDirty = true;
                ...
                {
                    i = i + 2;
                    llbMSGError.Text = "Excel Sheet : Has <b>OrderType</b> Field Have Not NB  At Row : " + i;
                    FlagIsDirty = true;
                    return;
                }
                ...
                if (chk.Checked == true)
                {
                  ...
                  decimal availableAmount = decimalDepositAmount - inttotcoll;
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        ...
                        try
                        { 
                            cmd.CommandText = "DataUpload_Insert_Vendor_hr";
                            ...
                            string strQuery = "select ChassisNo from Vendor_HSRPRecords where process='D' and remarks='allready in system'";
                            ...
                            else
                            {
                                ...
                                //if (lblMobileNO.Text.Trim().Length > 0)
                                //{
                                //    string chrcquery = "select   CashReceiptNo from hsrprecords where ChassisNo  = '" + lblChasisNo.Text.Trim().ToString() + "' and HSRP_StateID=4 ";
                                //    DataTable dtchrc = Utils.GetDataTable(chrcquery, CnnString);

                                //    string SMSText = "Received Rs. " + Math.Round(decimal.Parse(amt), 0) + " against HSRP For ChassisNo  No. " + lblChasisNo.Text.Trim().ToUpper() + " RECEIPT No. " + dtchrc.Rows[0]["CashReceiptNo"].ToString() + " dated  " + System.DateTime.Now.ToString("dd/MM/yyyy") + "-HSRP Team ";
                                //    string sendURL = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-hsrphr&password=hsrphr&type=0&dlr=1&destination=" + lblMobileNO.Text.ToString() + "&source=HRHSRP&message=" + SMSText;                                   
                                ...
                                //    Utils.ExecNonQuery("insert into HRSMSDetail(RtoLocationID,VehicleRegNo,MobileNo,SentResponseCode,smstext) values('" + RTOLocationID + "','" + lblvehicleregno.Text.Trim().ToUpper() + "'," + lblMobileNO.Text.ToString() + ",'" + result + "','" + SMSText + "')", CnnString);
                                //}
                            }
