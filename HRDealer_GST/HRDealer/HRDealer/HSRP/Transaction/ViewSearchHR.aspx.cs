    ...
    public partial class ViewSearchHR : System.Web.UI.Page
    {
        ...
        protected void Grid1_ItemCommand(object sender, ComponentArt.Web.UI.GridItemCommandEventArgs e)
        {
            ...
            if (Createdby.Trim().ToString() == Session["UID"].ToString().Trim() && Date == currenrtDate)
            {
                ...
                string Address;
                GetAddress = Utils.GetDataTable("select * from HSRPState WHERE HSRP_StateID='" + 4 + "'", CnnString);                            
                ...
                if (e.Control.ID.ToString() == "LinkButtonCashReceipt")
                {
                    ...
                    if (obj.FillHSRPRecordDeliveryChallan(HSRPRecordID, ref dataSetFillHSRPDeliveryChallan))
                    {
                        ...
                        string getTinNonew = Utils.getScalarValue("select TinNo from hsrpstate where HSRP_StateID='4' ", CnnString);
                        ...
                        string getTinNo = Utils.getScalarValue("select TinNo from hsrpstate where HSRP_StateID='4' ", CnnString);
        ...
        public void EpsionPrint(string hsrpRecordID)
        {
            ...
            if (dataSetFillHSRPDeliveryChallan.Rows.Count > 0)
            {
                ...
                string getTinNo = Utils.getScalarValue("select TinNo from hsrpstate where HSRP_StateID='4' ", ConnectionString);
