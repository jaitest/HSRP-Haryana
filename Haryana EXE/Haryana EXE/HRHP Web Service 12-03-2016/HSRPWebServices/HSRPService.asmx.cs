...
namespace HSRPWebServices
{
    ...
    {
        //SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["ConnectionStringAPP"].ToString());
        utils obj = new utils();
        ...
        #region GetvehicleRegNo
        [WebMethod]
        public DataTable GetvehicleRegNo()
        {
            string StrRecord = string.Empty;

            string sqlText = "select distinct VehicleregNo  from  hsrpexcelupload WHERE  ISNULL(VehicleregNo,'')!=''";

            string CnnString = obj.strProvider;
            DataTable dt = new DataTable();
            dt = utils.GetDataTable(sqlText, CnnString);

            dt.TableName = "hsrpexcelupload";
        ...
        public string InsertDataCashCollection(string HSRP_StateID, string RTOLocationID, string HSRPRecord_AuthorizationNo, string HSRPRecord_AuthorizationDate, string VehicleRegNo, string OwnerName, string ownerFatherName, string Address1, string MobileNo, string VehicleClass, string OrderType, string StickerMandatory, string isVIP, string NetAmount, string Roundoff_NetAmount, string VehicleType, string OrderStatus, string CashReceiptNo, string ChassisNo, string EngineNo, string DealerCode, string CreatedBy, string SaveMacAddress, string Addrecordby, string ISFrontPlateSize, string ISRearPlateSize, string FrontPlateSize, string RearPlateSize, string Reference, string ManufacturerModel, string vehicleref, string ManufacturerName, string CounterNo, string Affixid , DateTime  date)
        {
            ...
            if (Convert.ToString(date) != "")
            {
               ...
               DataTable dtvalue= utils.GetDataTable(queryrto, CnnString);

               if (!string.IsNullOrEmpty(dtvalue.Rows[0]["dealerstartdate"].ToString()))
                {
            ...
            else
            {
                ...
                {
                    if (VehicleType.ToUpper().Equals("SCOOTER") || VehicleType.ToUpper().Equals("MOTOR CYCLE") || VehicleType.ToUpper().Equals("TRACTOR") || VehicleType.ToUpper().Equals("E-RICKSHAW"))
                    {
                        StickerMandatory = "N";
                    }
                    else
                    ...
                    {  
                        using (SqlCommand cmd = new SqlCommand("Transaction_DataCashCollection_HR", con))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@VehicleRegNo", VehicleRegNo.Replace(" ", "").ToString());
                    ...
                    if (check > 0)
                    {
                        ...
                        Status = "Record Saved" + "^" + VehicleRegNo + "^" + HSRPRecord_AuthorizationNo;
                    }
                    else
                    {
        ...
        #region CheckDuplicateVehicleRegNo
        [WebMethod]
        public String CheckDuplicateVehicleRegNo( string VehicleRegNo, string Uid)
        {
            string StrRecord = string.Empty;
            ...
            DataTable dtAcuserid = utils.GetDataTable(query, CnnString);
            ...
            if (dtAcuserid.Rows.Count > 0)
            {
                for (int i = 0; i < dtAcuserid.Rows.Count; i++)
                {
                    ...
                    if (Uid == dtAcuserid.Rows[i]["userid"].ToString().Trim())
                    {

                        for (int j = 0; j < dtAcuserid.Rows.Count; j++)
                        {

                          string sqlquery = "select count (vehicleRegNo)as no from hsrprecords where  hsrp_stateid = 4 and  LTRIM(RTRIM(vehicleRegNo))= '" + VehicleRegNo.ToString().Trim() + "' and createdby = '" + dtAcuserid.Rows[j]["userid"].ToString().Trim() + "'";
        ...
        public String CheckDuplicateEntry(string HSRPStateID, string AuthNo, string VehicleRegNo, string OrderType)
        {
            ...
            string sqlText = "select COUNT(VehicleRegNo) as co from dbo.HSRPRecords  where hsrp_stateid ='" + HSRPStateID + "' and  LTRIM(RTRIM(VehicleRegNo)) ='" + VehicleRegNo.Replace(" ", "") + "' and OrderType='" + strOrderType + "' and  LTRIM(RTRIM(HSRPRecord_AuthorizationNo))='" + AuthNo.Replace(" ", "") + "'";
            string sqltextAuthorizationNo = "select COUNT(HSRPRecord_AuthorizationNo) as HSRPRecord_AuthorizationNo from hsrprecords where HSRPRecord_AuthorizationNo='" + AuthNo.Replace(" ", "") + "' and HSRP_StateID='" + HSRPStateID + "'";
            string sqltextOrderStatus = "select COUNT(OrderStatus) AS OrderStatus from hsrprecords where VehicleRegNo='" + VehicleRegNo.Replace(" ", "") + "' and hsrp_stateid ='" + HSRPStateID + "' and OrderStatus='New Order'";
            string sqltextValidateVehicleRegNoFromStaggingArea = "select top 1 * from HSRPRecordsStaggingArea where VehicleRegNo='" + VehicleRegNo.Replace(" ", "") + "' and HSRP_StateID='" + HSRPStateID + "' order by HSRPRecord_CreationDate desc";
            string CnnString = obj.strProvider;
            DataTable dt = new DataTable();
            dt = utils.GetDataTable(sqlText, CnnString);
            DataTable dtAuthorizationNo = new DataTable();
            dtAuthorizationNo = utils.GetDataTable(sqltextAuthorizationNo, CnnString);
            DataTable dtOrderStatus = new DataTable();
            dtOrderStatus = utils.GetDataTable(sqltextOrderStatus, CnnString);
            DataTable dtValidateVehicleRegNoFromStaggingArea = new DataTable();
            dtValidateVehicleRegNoFromStaggingArea = utils.GetDataTable(sqltextValidateVehicleRegNoFromStaggingArea, CnnString);
            //dt.Rows[0]["co"].ToString();            
            if (dtValidateVehicleRegNoFromStaggingArea.Rows.Count.ToString()=="0")
            {
                if (dtOrderStatus.Rows[0]["OrderStatus"].ToString() == "0")
                {
                    ...
                    {
                        StrRecord = "Pass";
                        if (dtAuthorizationNo.Rows[0]["HSRPRecord_AuthorizationNo"].ToString() != "0")
                        {
                            StrRecord = "HSRPRecord_AuthorizationNoDuplicate" + "^" + VehicleRegNo.ToString();
                        }
                ...
                {
                    StrRecord = "StatusAlreadyNew" + "^" + VehicleRegNo.ToString();
                }
            }
            else
            {
                StrRecord = "RecordExists" + "^" + VehicleRegNo.ToString();
            }
            return StrRecord;
        }
        #endregion
        ...
        #region AC TextBox Validation
        [WebMethod(Description = "AC TextBox Validation")]
        public string ACTextBoxValidation(string HSRPStateID, string USERID)
        {
            string CnnString = obj.strProvider;
            if (utils.GetDataTable("Select h.parametertype from HRParameter_master h where h.hsrp_stateid='" + HSRPStateID + "' and h.userid='" + USERID + "' and h.parametertype='HRCashCollectionCounterTextBox' and h.activestatus='Y'", CnnString).Rows.Count > 0)
            {
                return utils.GetDataTable("Select h.parametertype from HRParameter_master h where h.hsrp_stateid='" + HSRPStateID + "' and h.userid='" + USERID + "' and h.parametertype='HRCashCollectionCounterTextBox' and h.activestatus='Y'", CnnString).Rows[0]["parametertype"].ToString();
            }
            else
            {
                return "";
        ...
        #region AC TextBox Validation
        [WebMethod(Description = "Validation for AC and DepositAmount with CollectionAmount ")]
        public string ACandAmountValidation(string AcNo, string USERID, string HSRPStateID)
        {
            string CnnString = obj.strProvider;
            String SqlAc = "Select h.acno from HRAC_master h where h.hsrp_stateid='" + HSRPStateID + "' and h.userid='" + USERID + "' and h.acno='" + AcNo + "' and h.activestatus='Y'";
            DataTable dt1 = utils.GetDataTable(SqlAc, CnnString);
            string msg="";
            if (dt1.Rows.Count > 0)
            {                
                string Result = "";                
                ...
                return msg; //dt1.Rows[0]["acno"].ToString();                
            }
            else
            {                
                return "AC No. Invalid.";
            }
        }
        #endregion
    }
