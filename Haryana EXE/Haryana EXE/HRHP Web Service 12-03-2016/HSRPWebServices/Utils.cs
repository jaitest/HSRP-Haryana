...
namespace HSRPWebServices
{
    ...
    class utils
    {
        ...
        public static string CnnString = System.Configuration.ConfigurationSettings.AppSettings["ConnectionStringAPP"].ToString();
        ...
        public static int ExecNonQuery(string query,SqlParameter [] sqlParamerets,string CnnnString)
        {
            int count = 0;
            using (SqlConnection connection = new SqlConnection(CnnString))
            {
                ...
                if (sqlParamerets != null)
                    command.Parameters.AddRange(sqlParamerets);

                command.Connection.Open();
                count = command.ExecuteNonQuery();
                count = Convert.ToInt32(command.Parameters["@NewId"].Value);
                command.Connection.Close();
                 return count;
            }            
        }
