...
{
    public partial class APTGDataSyn : System.Web.UI.Page
    {
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string SQLString = "select rtolocationname as [RTOName],count(*) as Performance from hsrprecords a,rtolocation b where a.RTOLocationID=b.rtolocationid and a.HSRP_StateID='" + DropDownListStateName.SelectedValue.ToString() + "' and RecievedAtAffixationDateTime between '" + AuthorizationDate + "' and '" + ToDate + "'group by rtolocationname";
                 SQLString = "update hsrprecords set sendtoerp=Null, remarks='ERP' ,NAVSalesOrderNo=Null,NAVProdOrderNo=Null where hsrprecordid in(select HSRPRecordID from hsrprecords where HSRP_StateID='"+DropDownListStateName.SelectedValue.ToString()+"' and isnull(VehicleRegNo,'')!='' and erpassigndate is null and isnull(hsrp_front_lasercode,'')=''and OrderStatus='New Order' and hsrprecord_creationdate between '" + AuthorizationDate + "' and '" + ToDate + "')";
                count =Utils.ExecNonQuery(SQLString, CnnString);
                //count = Convert.ToInt32(cmd.ExecuteScalar());

                lblmsg.Text = "Total Record update"  + count;
                //if (dt.Rows.Count > 0)
                //{
