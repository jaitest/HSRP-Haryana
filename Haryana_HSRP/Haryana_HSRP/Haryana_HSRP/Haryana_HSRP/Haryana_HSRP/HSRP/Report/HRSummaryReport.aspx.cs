...
{
    public partial class HRSummaryReport : System.Web.UI.Page
{
    int UserType1;
    ...
    private void Export(string strStateId, string strStateName)
    {
        try
        {
            ...
            book.Properties.Title = "HR Summary";
          ...
          string  FromDate = OrderDate.SelectedDate.ToString("MM/dd/yy 00:00:00");
           string ToDate = HSRPAuthDate.SelectedDate.ToString("MM/dd/yy 23:59:59");
            if (DropDownListStateName.SelectedValue == "4")
            {
                SqlCommand cmd = new SqlCommand("[Report_AllStateLocationWise_New]", con);
            ...
            //else if (DropDownListStateName.SelectedValue == "11")
            //{
            //    SqlCommand cmd = new SqlCommand("[Report_APTSLocationwise]", con);
            //    cmd.CommandType = CommandType.StoredProcedure;
            //    cmd.Parameters.Add(new SqlParameter("@stateid", strStateId));
            //    cmd.Parameters.Add(new SqlParameter("@datefrom", FromDate));
            //    cmd.Parameters.Add(new SqlParameter("@dateto", ToDate));
            ...
            row.Cells.Add(new WorksheetCell("Report :", "HeaderStyle3"));
                ...
                row.Cells.Add(new WorksheetCell("Status Report", "HeaderStyle3"));           
            ...
            for (int j = 0; j < dt.Rows.Count; j++)
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    if (dt.Rows[j]["locationname"].ToString().Equals(" : -") && i.Equals(0))
                    {
                        AddNewCell(row, "Total", "HeaderStyle6", 1);
                    }
                    else if(dt.Rows[j]["locationname"].ToString().Equals(1))
                    {
                        AddNewCell(row, "", "HeaderStyle2", 1);
                    }
                    else
                    {
                        if (dt.Columns[i].ColumnName.ToString().Equals("ID"))
                        {
                        ...
                        else
                        {
                            if (i == 1)
                            {
                                AddNewCell(row, dt.Rows[j][1].ToString(), "HeaderStyle2", 1);
                            }
                            else
                            {
                                AddNewCell(row, dt.Rows[j][i].ToString(), "HeaderStyle6", 1);
