...
namespace HSRP.EmbossingData
{
    public partial class ReprintProdSheet : System.Web.UI.Page
    {
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
            ...
            string RTOLocationID = dropDownListClient.SelectedValue.ToString().Trim();
            ...
            if (RTOLocationID == "698")
                ...
                else
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (RTOLocationID == "698")
                        {
                            filename = "HSRPProductionSheet-Chandigarh SDM " + System.DateTime.Now.Day.ToString() + System.DateTime.Now.Month.ToString() + System.DateTime.Now.Year.ToString() + System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + ".pdf";
                        }
                        else
                        {
                        ...
                        string que = " select  Distinct  dealerid   from hsrprecords  where hsrp_StateID='"+strStateID+"' and  rtolocationid='"+RTOLocationID+"' and newpdfrunningno='"+dropdownDuplicateFIle.SelectedItem+"'";
                        DataTable dtdealerid = Utils.GetDataTable(que.ToString(), CnnString);

                        if (!string.IsNullOrEmpty(dtdealerid.Rows[0]["dealerid"].ToString().Trim()))
                        {
                        string quer = "select dealerid ,dealername   from  Dealermaster   where dealerid ='"+dtdealerid.Rows[0]["dealerid"].ToString()+"' and hsrp_stateid = '"+strStateID+"' ";
                        DataTable dtdealeridname = Utils.GetDataTable(quer.ToString(), CnnString);

                        PdfPCell cell786a1 = new PdfPCell(new Phrase("Dealer Id /Name:  " + dtdealeridname.Rows[0]["dealerid"].ToString() +"/"+ dtdealeridname.Rows[0]["dealername"].ToString().ToUpper(), new iTextSharp.text.Font(bfTimes, 10f, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)));
                        cell786a1.Colspan = 14;
                        cell786a1.BorderWidthLeft = 0f;
                        cell786a1.BorderWidthRight = 0f;
                        cell786a1.BorderWidthTop = 0f;
                        cell786a1.BorderWidthBottom = 0f;                       
                        cell786a1.HorizontalAlignment = 2; //0=Left, 1=Centre, 2=Right
                        table.AddCell(cell786a1); 
                        }
                        ...
                        if (HSRPStateID == "1" ||  HSRPStateID == "3" || HSRPStateID == "6" ||  HSRPStateID == "4" )
                        {
