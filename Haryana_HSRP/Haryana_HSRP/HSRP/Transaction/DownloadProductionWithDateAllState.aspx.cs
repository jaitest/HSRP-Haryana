...
namespace HSRP.Master
{
    public partial class DownloadProductionWithDateAllState : System.Web.UI.Page
    {
        ...
        protected void btnDownloadRecords_Click(object sender, EventArgs e)
        {
            ...
            if (strOrderTypeHHTDealer == "--Select Data Type--")
            {
                lblErrMsg.Visible = true;
                lblErrMsg.Text = "";
            ...
            if (strStateID == "4" || strStateID == "1")
            {
                ...
                {
                    strSQL.Append(" AND a.dealerid = '" + DDlDealerName.SelectedValue.ToString() + "' ");
                    
                }

                else if (ddlBothDealerHHT.SelectedItem.Text == "Other")
                {
                    strSQL.Append(" AND isnull(a.Dealerid,'')=''");
        ...
        {
            string RTOLocationID = dropDownListClient.SelectedValue.ToString();
            SQLString = "select dealerid,dealername from dealermaster where  dealerid in ( select distinct Dealerid from hsrprecords  where HSRP_StateID=" + DropDownListStateName.SelectedValue.ToString() + " and rtolocationid = '" + dropDownListClient.SelectedValue.ToString() + "' and   convert(date,erpassigndate) = convert(date,'" + OrderDate.SelectedDate.ToString() + "') and isnull(Dealerid,'')!='' )Order by dealername";
            ...
            {
                ddlBothDealerHHT.SelectedValue = "--Select Data Type--";
