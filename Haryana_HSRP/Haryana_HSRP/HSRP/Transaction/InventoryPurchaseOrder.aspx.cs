...
{
    public partial class InventoryPurchaseOrder : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            Int64 totalPlateInbox = Convert.ToInt64(LaserCodeTo) - Convert.ToInt64(textboxLaserCodeFrom.Text);
            BAL obj = new BAL();
            int IsExists = -1;

            int totalRecord = 0;
            int TotalExistRecord = 0;
            Int64 Laser = Convert.ToInt64(textboxLaserCodeFrom.Text) - 1;
            for (int i = 0; i <= totalPlateInbox; i++)
            {
                Laser = +Laser + 1;
                string LaserNo = DropDownListPrefixLaserNo.SelectedItem.Text + Laser.ToString();
                obj.InsertHSRPPInventoryPurchase(ProductID, UserID, textboxLaserCodeFrom.Text, Prefix, textboxQuantity.Text, CurrentCost, Weight, textboxInvNo.Text, textboxInvFrom.Text, intHSRPStateID, intRTOLocationID, Remarks, Laser, LaserNo, orderStatus, ref IsExists);
                if (IsExists.Equals(1))
                {
                    ...
                    SQLString = "select  LaserNo, (select hsrpStateName from hsrpstate where hsrp_StateID= a.hsrp_StateID) as StateName from RTOInventorypurchase a where a.LaserNo='" + LaserNo + "'";
