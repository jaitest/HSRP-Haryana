    ...
    public partial class UploadDealerData : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (DropDownListStateName.SelectedItem.ToString().Equals("--Select State--"))
                {
                    ...
                    llbMSGError0.Text = "";
                    llbMSGError0.Text = "Please Select State...";
                    return;
                }
            ...
            if (dropDownListClient.SelectedItem.ToString().Equals("--Select Location--"))
            {
                llbMSGError0.Visible = true;
                llbMSGError0.Text = "";
                llbMSGError0.Text = "Please Select Rto Location...";
                return;
            }
        ...
        private void InsertionOfRecords(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string strVehicle = "select count(*) from hsrprecords where LTRIM(RTRIM(hsrp_stateid))= LTRIM(RTRIM('" + DropDownListStateName.SelectedValue.ToString().Trim() + "')) and LTRIM(RTRIM(vehicleregno))= LTRIM(RTRIM('" + tt + "')) and LTRIM(RTRIM(OrderType))= LTRIM(RTRIM('" + OrderType.Trim() + "'))";
                ...
                string SQLString1 = "select vehicleRegNo from dbo.Vendor_HSRPRecords where LTRIM(RTRIM(vehicleRegNo))= LTRIM(RTRIM('" + VehicleRegNo.ToString().Trim() + "'))";
        ...
        private void ValidationCheckOnRecords(DataTable ExcelSheet)
        {
            if (DropDownListStateName.SelectedValue.ToString() == "4")
            {
                for (int i = 0; i < ExcelSheet.Rows.Count; i++)
                {
                    ...
                    string strCheck = strVehicleNo.Trim().ToUpper().Substring(0, 2);
                    if (strCheck != "HR")
                    {
                        i = i + 2;
                        llbMSGError.Text = "Please Enter Valid VehicleRegNo At Row : " + i;
                        FlagIsDirty = true;
                        return;
                    }
            ...
            if (DropDownListStateName.SelectedValue.ToString() == "1")
            {
                for (int i = 0; i < ExcelSheet.Rows.Count; i++)
                {
                    string strVehicleNo = ExcelSheet.Rows[i]["VehicleRegNo"].ToString();
        ...
        protected void btnSync_Click(object sender, EventArgs e)
        {
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@stateId", Convert.ToInt32(DropDownListStateName.SelectedValue.ToString()));
