...
{
    public partial class Chalan1 : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            {
                string SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
                    "from hsrprecords where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" +
                    dropDownListClient.SelectedValue + "' and PdfFileName='" + dropdownDuplicateFIle.SelectedItem.ToString() +
                    "' and orderstatus ='New Order' and hsrp_rear_lasercode is not null and hsrp_Front_lasercode is not null " +
                    "and  hsrp_rear_lasercode<>'' and hsrp_Front_lasercode<>'' and hsrp_rear_lasercode<>'NULL' and hsrp_Front_lasercode<>'NULL' order by VehicleClass,VehicleType,VehicleRegNo";
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    SQLString = "select  Distinct pdffilename   from hsrprecords  where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" + dropDownListClient.SelectedValue + "' and  pdffilename is not null and hsrprecord_creationdate between '" + strFrmDateString + "' and  '" + strToDateString + "' and HSRP_Front_LaserCode is not null and HSRP_Rear_LaserCode is not null and  hsrp_rear_lasercode<>'' and hsrp_Front_lasercode<>'' and orderstatus='New Order' group by  pdffilename, convert(varchar,pdfdownloaddate,111)";
            ...
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        string strUpdate = "update hsrprecords set ChallanNo='" + strInvoiceNo + "',challandate=getdate()," +
                               " orderstatus='Embossing Done',[OrderEmbossingDate]=getdate(),embossinguserid='" + strUserID + "' where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and hsrprecordid ='" + strRecordId + "' ";
                        Utils.ExecNonQuery(strUpdate, CnnString);

                        //strUpdate = "update rtoinventory set HSRPRecordID='" + strRecordId + "',InventoryStatus='Embossing Done',EmbossingDate=getdate() where  hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and LaserNo = (select hsrp_front_lasercode from hsrprecords where hsrprecordid='" + strRecordId + "') or LaserNo = (select hsrp_rear_lasercode from hsrprecords where hsrprecordid='" + strRecordId + "') ";
                       // Utils.ExecNonQuery(strUpdate, CnnString);

                        LblMessage.Visible = true;
                        LblMessage.Text = "Vehicles Embossed Successfully.....";
                ...
                string sqlstring = string.Empty;
                sqlstring = "insert into InvoiceMaster(InvoiceNo,InvoiceDate,Amount,BuyerName,clientName,hsrp_stateid) values('" + strInvoiceNo + "', Convert(date,('" + currentdate + "'),103),'" + totalamount + "','" + RtoName + "','" + GetAddress.Rows[0]["Address1"].ToString() + "','" + DropDownListStateName.SelectedValue + "')";
                ...
                if (true)
                {
                    ...
                    GenerateCell(table, 1, 1, 1, 1, 0, 1, 0, "Qty-:Plate", 0, 0);
