        ...
        else
        {
            ...
            try
            {
                ...
                row.Cells.Add(new WorksheetCell("", "HeaderStyle2"));
                            ...
                            row.Cells.Add(new WorksheetCell("Challan No", "HeaderStyle6"));
                            row.Cells.Add(new WorksheetCell("Challan Date", "HeaderStyle6"));
                            row.Cells.Add(new WorksheetCell("User Name", "HeaderStyle6"));
                            ...
                            SQLString1 = "select r.RTOLocationName,VehicleRegNo,convert(varchar(20),OrderDate,103) as OrderDate,convert(varchar(20),OrderEmbossingDate,103) as OrderEmbossingDate ,DealerId,VehicleType,HSRP_Front_LaserCode,HSRP_Rear_LaserCode,OrderStatus, ChallanNo,ChallanDate ,(select   userfirstname +' '+userlastname from users   where userid = createdby) as username  from HSRPRecords inner join rtolocation as r on r.rtolocationid=hsrprecords.rtolocationid  where hsrprecords.hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and addrecordby='Dealer' and HsrpRecord_creationdate between '" + ReportDate1 + "' and '" + ReportDate2 + "' order by r.rtolocationname";
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow dtrows in dt.Rows) // Loop over the rows.
                                {
                                    ...
                                    row.Cells.Add(new WorksheetCell(dtrows["ChallanNo"].ToString(), DataType.String, "HeaderStyle"));

                                    row.Cells.Add(new WorksheetCell(dtrows["ChallanDate"].ToString(), DataType.String, "HeaderStyle"));
