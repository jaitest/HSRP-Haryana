...
{
    public partial class OnlineCashCollection : System.Web.UI.Page
    {
        string CnnString = string.Empty;
        string SQLString = string.Empty;
        string strUserID = string.Empty;
        string ComputerIP = string.Empty;
        string rtolcation = string.Empty;
        ...
        private void FilldropDownListOrganization()
        {
            if (UserType.Equals(0))
            {
                SQLString = "select HSRPStateName,HSRP_StateID from HSRPState  where ActiveStatus='Y' and HSRP_StateID='5' Order by HSRPStateName";
        ...
        protected void btnExportToExcel_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               // int.TryParse(dropDownListClient.SelectedValue, out intRTOLocationID);
                ...
                string filename = "Online Cash Collection-" + System.DateTime.Now.Day.ToString() + System.DateTime.Now.Month.ToString() + System.DateTime.Now.Year.ToString() + System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + ".xls";
                ...
                Worksheet sheet = book.Worksheets.Add("HSRP Online Cash Collection Report");
                ...
                WorksheetCell cell = row.Cells.Add("HSRP Online Cash Collection Report");
               ...
               // row.Cells.Add(new WorksheetCell("Record Type :", "HeaderStyle2"));
                ...
                row.Cells.Add(new WorksheetCell("Payment Order", "HeaderStyle6"));
                row.Cells.Add(new WorksheetCell("Payment Date", "HeaderStyle6"));
                row.Cells.Add(new WorksheetCell("Net Amount", "HeaderStyle6"));
                ...
                UserType = Convert.ToInt32(Session["UserType"]);
                int.TryParse(DropDownListStateName.SelectedValue, out intHSRPStateID);
                //string SQLQuery = "select rto_name from View_MP_HSRPRecordsStagging_New where vehicleregno in (select registrationno from VehicleOwner_Web where PaymentDateTime between '" + start + "' and '" + End + "')";
                //DataTable dt2 = Utils.GetDataTable(SQLQuery, CnnString);
               //for(int i=0; i<=dt2.Rows.Count;i++)
               // {
               //  rtolcation = dt2.Rows[i]["rto_name"].ToString();
               // }
                strquery = "select distinct w.RegistrationNo, v.rto_name as RTOName,w.VehicleClass,w.VehicleType,w.Ordertype,w.Chassisno,w.EngineNo,w.OwnerName,w.PhoneNo,w.PaymentOrderID, convert(varchar(12),w.PaymentDateTime,103) as PaymentDateTime,w.NetAmount from VehicleOwner_Web as W left join View_MP_HSRPRecordsStagging_New as v on v.VehicleRegNo= w.RegistrationNo where w.PaymentStatus='Y' and w.RegistrationNo is not null and  w.PaymentDateTime between  '" + start + "' and '" + End + "' order by w.RegistrationNo";
                //strquery = " select RegistrationNo,VehicleClass,VehicleType,Ordertype,Chassisno,EngineNo,OwnerName,PhoneNo,PaymentOrderID, convert(varchar(12),PaymentDateTime,103) as PaymentDateTime from VehicleOwner_Web where PaymentStatus='Y' and PaymentDateTime between '" + start + "' and '" + End + "'";
                DataTable dt1 = Utils.GetDataTable(strquery, CnnString);
                if(dt1.Rows.Count>0)
                {
                    int sno = 0;
                    foreach (DataRow dtrows in dt1.Rows) // Loop over the rows.
                    {
                        ...
                        row.Cells.Add(new WorksheetCell(dtrows["RegistrationNo"].ToString(), DataType.String, "HeaderStyle"));
                        row.Cells.Add(new WorksheetCell(dtrows["RTOName"].ToString(), DataType.String, "HeaderStyle"));
                        row.Cells.Add(new WorksheetCell(dtrows["VehicleClass"].ToString(), DataType.String, "HeaderStyle2"));
                        row.Cells.Add(new WorksheetCell(dtrows["VehicleType"].ToString(), DataType.String, "HeaderStyle"));
                        row.Cells.Add(new WorksheetCell(dtrows["Ordertype"].ToString(), DataType.String, "HeaderStyle"));
                        ...
                        row.Cells.Add(new WorksheetCell(dtrows["PhoneNo"].ToString(), DataType.String, "HeaderStyle"));
                        row.Cells.Add(new WorksheetCell(dtrows["PaymentOrderID"].ToString(), DataType.String, "HeaderStyle"));
                        row.Cells.Add(new WorksheetCell(dtrows["PaymentDateTime"].ToString(), DataType.String, "HeaderStyle"));
