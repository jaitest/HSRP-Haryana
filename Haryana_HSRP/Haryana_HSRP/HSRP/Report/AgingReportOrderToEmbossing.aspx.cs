...
{
    public partial class AgingReportOrderToEmbossing : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            if (DropDownListStateName.SelectedItem.Text != "--Select State--")
            {
                SQLString = "select row_number() over(order by r.RtoLocationName) as Sno,r.RtoLocationName,isnull(sum(case when h.HSRPRecord_CreationDate > (Convert(datetime, '" + AuthorizationDate + "')-7) then 1 end),0) as '7 days'," +
                            ...
                            "hsrprecords h inner join rtolocation r on h.rtolocationid=r.rtolocationid where h.hsrp_stateid='" + DropDownListStateName.SelectedValue + "' and h.orderstatus='Embossing Done' group by r.RtoLocationname order by r.RtoLocationname ";
                DataTable dts = Utils.GetDataTable(SQLString, CnnString);
                // .DataSource = dts;
