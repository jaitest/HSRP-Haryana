        ...
        protected void btnexport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    if (DdlreportType.SelectedValue.ToString() == "Center")
                    {
                        SQLString = "SELECT t.[TransactionID] ,convert(varchar, t.DepositDate, 103) AS DepositDate,m.[BankName],(select RTOLocationName from RTOLocation where rtolocationid=t.RTOLocation) as Center,t.[BranchName],t.[DepositAmount],t.[DepositBy],t.[StateID],t.[RTOLocation],convert(varchar, EntryDate, 103) as EntryDate,t.[BankSlipNo],t.[Remarks],t.[AccountNo],Chq_no,chq_date,(select (userfirstname+' '+UserLastName) as Name from users u where t.userid=u.userid) as username FROM [dbo].[BankTransaction] as t inner join bankmaster as m on t.BankName=m.id  WHERE t.EntryDate between '" + ReportDate1 + "' and '" + ReportDate2 + "' and  t.StateID='" + DropDownListStateName.SelectedValue.ToString() + "'  and  isnull(voidstatus ,'')='' AND t.RTOLocation='" + dropDownListClient.SelectedValue.ToString() + "'  and  userid in(select userid from users where isnull(dealerid,'')='')  order by t.EntryDate desc";
                    }
                    if (DdlreportType.SelectedValue.ToString() == "Dealer")
                    {
                        SQLString = "SELECT t.[TransactionID] ,convert(varchar, t.DepositDate, 103) AS DepositDate,m.[BankName],(select RTOLocationName from RTOLocation where rtolocationid=t.RTOLocation) as Center,t.[BranchName],t.[DepositAmount],t.[DepositBy],t.[StateID],t.[RTOLocation],convert(varchar, EntryDate, 103) as EntryDate,t.[BankSlipNo],t.[Remarks],t.[AccountNo],Chq_no,chq_date,(select (userfirstname+' '+UserLastName) as Name from users u where t.userid=u.userid) as username FROM [dbo].[BankTransaction] as t inner join bankmaster as m on t.BankName=m.id  WHERE t.EntryDate between '" + ReportDate1 + "' and '" + ReportDate2 + "' and  t.StateID='" + DropDownListStateName.SelectedValue.ToString() + "'  and  isnull(voidstatus ,'')='' AND t.RTOLocation='" + dropDownListClient.SelectedValue.ToString() + "'  and userid in(select userid from users where isnull(dealerid,'')!='')  order by t.EntryDate desc";
