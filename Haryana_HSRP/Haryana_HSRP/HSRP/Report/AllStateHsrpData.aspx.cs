...
{
    public partial class AllStateHsrpData : System.Web.UI.Page
    {
          ...
          protected void btnGO_Click(object sender, EventArgs e)
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (ddllasercode.SelectedItem.Text == "HSRPRECORDID")
                        {
                            lblSucessMsg.Text = "";
                            lblErrMsg.Text = "";
                            ...
                            for (j = 0; j < TextBoxValue.Length; j++)
                            {
                                StringBuilder bb = new StringBuilder();

                                foreach (var itm in TextBoxValue)
                                {     
                                    //if (itm.Selected.Equals(true))
                                    //{
                                    //    if (itm.Value.Equals("--Select Vehicle--"))
                                    //    {
                                    //        System.Windows.Forms.MessageBox.Show("Selected item is not valid.");
                                    //        return;
                                    //    }

                                    bb.Append(itm);
                                    if (j == TextBoxValue.Length-1)
                                    {
                                    ...
                                    //dt.Rows.Add("hsrprecords");
                                }

                                SQLString = "select * from hsrprecords where HSRPRecordID in(" + bb + ")";
                    ...
                    if (ddllasercode.SelectedItem.Text == "VehicleRegNo")
                    {
                        lblSucessMsg.Text = "";
                        lblErrMsg.Text = "";
                        string UID = Session["UID"].ToString();
                        ...
                        for (int j = 0; j < TextBoxValue.Length; j++)
                        {
                            ...
                            SQLString = "insert into HSRP_Laser_Removal(HSRPRecordID,HSRP_Front_LaserCode,UserID,RemovalDatetime,Remarks)values('" + TextBoxValue[j] + "','" + strHsrp_front_lasercode + "','" + UID + "','" + strRemovelDateTime + "')";
