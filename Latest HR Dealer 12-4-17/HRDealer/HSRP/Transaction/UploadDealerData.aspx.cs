    ...
    public partial class UploadDealerData : System.Web.UI.Page
    {
        ...
        private void InsertionOfRecords(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string strVehicle = "select count(*) from hsrprecords where LTRIM(RTRIM(hsrp_stateid))= LTRIM(RTRIM('4')) and LTRIM(RTRIM(engineno))= LTRIM(RTRIM('" + EngineNo + "')) and LTRIM(RTRIM(OrderType))= LTRIM(RTRIM('" + OrderType.Trim() + "'))";
                int Iresult = Utils.getScalarCount(strVehicle, CnnStringupload);
                if (Iresult > 0)
                {
                    ArrVehicle = ArrVehicle + "|" + EngineNo;
                ...
                string SQLString1 = "select engineno from dbo.Vendor_HSRPRecords where LTRIM(RTRIM(engineno))= LTRIM(RTRIM('" + EngineNo.ToString().Trim() + "'))";
               DataTable duplicatecount = Utils.GetDataTable(SQLString1, CnnStringupload);
               if (duplicatecount.Rows.Count > 0)
                {
                    countDuplicate = countDuplicate + 1;
                    lbltotladuplicaterecords.Text = duplicatecount.Rows[0]["engineno"].ToString();
                }
                else
                {
                    ...
                    SQLString2 = "select dealerid  from dealermaster where dealerid='" + DealerID+ "'";
                     ...
                     if (dt1.Rows.Count > 0)
                     {
                        ...
                        
                        {
                            ...
                            {
                                lblengineNo.Text += Engine.ToString() + "\n";
                    ...
                    if (string.IsNullOrEmpty(BlockVehicle))
                    {
                        ...
                        {
                            errorinexcel++;
                            llbMSGError.Text = "Engine No" + " " + EngineNo + " Has Some Error in Vehicle Type/Vehicle Class/Order Type";
                        }
        ...
        private void ValidationCheckOnRecords(DataTable ExcelSheet)
        {
            for (int i = 0; i < ExcelSheet.Rows.Count; i++)
            {
                ...
                for (int jj = 0; jj < ExcelSheet.Rows.Count; jj++)
                {
                    ...
                    {
                        if (ExcelSheet.Rows[i]["EngineNo"].ToString().Trim().ToUpper() == ExcelSheet.Rows[jj]["EngineNo"].ToString().Trim().ToUpper())
                        {
                            i = i + 2;
                            llbMSGError.Text = "Excel Sheet : Has <b>Engine No</b> Duplicate At Row : " + i;
                            FlagIsDirty = true;
                            return;
        ...
        {           
            ...
            { 
                ...
                if (chk.Checked == true)
                {
                    ...
                    Label lblengine = GridView1.Rows[i].Cells[1].FindControl("lblengineno") as Label;                           
                  ...
                  decimal availableAmount = decimalDepositAmount - inttotcoll;
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        ...
                        { 
                            ...
                            cmd.Parameters.AddWithValue("@engineeno", lblengine.Text.Trim().ToString());                            
                            cmd.ExecuteNonQuery();
                            //HRDealerRecord();                           
                            string strQuery = "select engineno from Vendor_HSRPRecords where process='D' and remarks='allready in system'";
                            ...
                            else
                            {
                                ...
                                {
                                    string chrcquery = "select   CashReceiptNo from hsrprecords where engineno = '" + lblengine.Text.Trim().ToString() + "' and HSRP_StateID=4 ";
                                    DataTable dtchrc = Utils.GetDataTable(chrcquery, CnnString);

                                    string SMSText = "Received Rs. " + Math.Round(decimal.Parse(amt), 0) + " against HSRP For Engine No. " + lblengine.Text.Trim().ToUpper() + " RECEIPT No. " + dtchrc.Rows[0]["dtchrc"].ToString() + " dated  " + System.DateTime.Now.ToString("dd/MM/yyyy") + "-HSRP Team ";
