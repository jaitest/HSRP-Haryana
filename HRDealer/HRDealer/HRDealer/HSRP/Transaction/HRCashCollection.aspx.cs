    ...
    public partial class HRCashCollection : System.Web.UI.Page
    {
        ...
        public void HRDealerRecord()
        {
            try
            {
                ...
                {
                    Session["Availableamt"] = Convert.ToUInt32(strDepositAmount) - Convert.ToUInt32(TodayCollection);
                }
                ...
                lblAvailableamt.Text = Session["Availableamt"].ToString();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                authdate = arrauthdate1[0].ToString();
                  ...
                  string SQLString1 = "select vehicleRegNo from dbo.Vendor_HSRPRecords where LTRIM(RTRIM(vehicleRegNo))= LTRIM(RTRIM('" +lblRegNo.Text.Trim().ToUpper()+ "'))";
               DataTable dtcount = Utils.GetDataTable(SQLString1,ConnectionString);
               if (dtcount.Rows.Count > 0)
                ...
                else
                {
                   ...
                   if ( Convert.ToInt32(Session["Availableamt"].ToString()) < Convert.ToInt32( Math.Round(decimal.Parse(lblAmount.Text), 0)))
                   {
                       lblErrMess.Visible = true;
                       lblErrMess.Text = "";
                       lblErrMess.Text = "Please  Contact With Administrator";
