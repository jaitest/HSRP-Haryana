    ...
    public partial class ViewDealer : System.Web.UI.Page
    {
        ...
        protected void btnviewdealer_Click(object sender, EventArgs e)
        {
                try
                {
                    g3.Visible = true;
                    g1.Visible = false;
                    SQLString = "SELECT [DealerID],[DealerName] +'-'+ ISNULL ([DealerCode],'') as Dealer,ISNULL ([Address],'') + space(2)+ISNULL ([City],'')+space(2)+ ISNULL ([State],'') as Address,ISNULL ([ContactPerson],'') as ContactPerson,ISNULL ([ContactMobileNo],'') as ContactMobileNo,ISNULL (AreaOfDealer,'') as AreaOfDealer,case when IsActive =1 then 'Active' else 'Inactive' end as ActiveStatus  FROM [DealerMaster]   where state= 'Haryana' and IsActive='1'";
                    DataTable dt = Utils.GetDataTable(SQLString, CnnString);
                    Grid3.DataSource = dt;
