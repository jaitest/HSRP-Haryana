...
namespace HSRP.Transaction
{
    public partial class HRCashRecept : System.Web.UI.Page
    {
        ...
        private void FilldropDownListUserNames()
        {
            try
            {
               ...
               //SQLString = "select p.userid,u.UserFirstName +' '+u.UserLastName+' '+u.UserLoginName as UserName from HRParameter_master p,users u where p.activestatus='Y' and p.hsrp_stateid='" + HSRPStateID + "' and p.userid=u.UserID and p.userid='"+ USERID +"' order by UserName ASC";
                SQLString = "select u.userid,u.UserFirstName +' '+u.UserLastName+' '+u.UserLoginName as UserName from dealermaster d,users u where u.activestatus='Y' and u.hsrp_stateid='4' and d.dealerid=u.dealerid  and u.UserID='" + USERID+ "' order by UserName ASC";   
                DataSet dts = Utils.getDataSet(SQLString, ConnectionString);
                ddlUserName.DataSource = dts;
                ddlUserName.DataTextField = "UserName";
                ddlUserName.DataValueField = "userid";
        ...
        public void EpsionPrint()
        {
            ...
            string SQLString = "select hsrprecordID,CashReceiptNo, HSRPRecord_CreationDate,HSRP_StateID,RTOLocationID,HSRPRecord_AuthorizationNo,HSRPRecord_AuthorizationDate,VehicleRegNo,convert(varchar(12),OwnerName)as OwnerName,MobileNo,VehicleClass,OrderType,StickerMandatory,IsVIP,NetAmount,VehicleType,engineno from hsrprecords where CreatedBy='" + ddlUserName.SelectedValue + "'  and HSRP_StateID=4 and addrecordby='Dealer' and APwebserviceresp is null order by  HSRPRecord_CreationDate desc";
            ...
            if (dataSetFillHSRPDeliveryChallan.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dataSetFillHSRPDeliveryChallan.Rows.Count; i++)
                {
                    ...
                    ///changes
                    PdfPCell cell100 = new PdfPCell(new Phrase("Engine No", new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                    cell9.Colspan = 0;
                    cell9.BorderWidthLeft = 0f;
                    cell9.BorderWidthRight = 0f;
                    cell9.BorderWidthTop = 0f;
                    ...
                    PdfPCell cell950 = new PdfPCell(new Phrase(": " + dataSetFillHSRPDeliveryChallan.Rows[i]["engineno"].ToString(), new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                    cell95.Colspan = 0;
                    cell95.BorderWidthLeft = 0f;
                    cell95.BorderWidthRight = 0f;
                    cell95.BorderWidthTop = 0f;
