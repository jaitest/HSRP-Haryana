...
namespace HSRP.Transaction
{
    ...
    public partial class ViewBankTransactionApprovalDealer : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (Session["UID"].ToString() == "1" || Session["UID"].ToString() == "7719" || Session["UID"].ToString() == "7449")
                {

                lblSearchPreference.Visible = true;
                ...
                else
                {
                    ...
                    lblErrMsg.Text = "You are  not  authorised  user.";
                    lblSearchPreference.Visible = false;
        ...
        private void Filldropdowndealer(string preference)
        {
            if (preference == "Dealerid")
            {
                ...
                ddlBothDealerHHT.Items.Insert(0, new ListItem("--Select Dealer--", "--Select Dealer--"));
            }
            else
            {
        ...
        public void buildGrid()
        {
            ...
            try
            {
                ...
                else if (ddlBothDealerHHT.SelectedValue.ToString() != "--Select Dealer--")
                {
                ...
                if (TransactionID != "")
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                string SMSText = "Dear Customer, A cheque of Rs. " + dt.Rows[0]["DepositAmount"].ToString() + " has been received against your account ID " + dt.Rows[0]["UserLoginName"].ToString() + ". Cheque subject to realization. Thank You ";
                                string sendURL = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-hsrphr&password=hsrphr&type=0&dlr=1&destination=" + MobileNo.ToString() + "&source=HRHSRP&message=" + SMSText;
                                ...
                                Utils.ExecNonQuery("insert into HRSMSDetail(RtoLocationID,MobileNo,SentResponseCode,smstext) values('" + dt.Rows[0]["rtolocationid"].ToString() + "'," + MobileNo.ToString() + ",'" + result + "','" + SMSText + "')", CnnString);
                               
                            }                        

                            lblSucMess.Text = "Cheqeue Approved Successfully...";
                            buildGrid();
                        }
                    ...
                    {
                        lblSucMess.Text = "Cheqeue not Approved..";
                    }
        ...
        protected void btngo_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (ddlBothDealerHHT.SelectedValue.ToString() == "--Select Dealer--")
                    {
                        lblErrMsg.Visible = true;
                        lblErrMsg.Text = "";
        ...
        protected void btnApproval(object sender, EventArgs e)
        {
            ...
            if (TransactionID != "")
            {
                ...
                if (i > 0)
                {
                    ...
                    {
                        string MobileNo = dt.Rows[0]["MobileNo"].ToString();                    
                         if (HSRPStateID.ToString() == "4")
                         {
                             if (MobileNo.Length > 0)
                             {
                                string SMSText = "Dear Customer, amount of Rs. " + dt.Rows[0]["DepositAmount"].ToString() + " has been credited to your account ID " + dt.Rows[0]["UserLoginName"].ToString() + " for a Cheque/RTGS payment. Transaction ID - " + dt.Rows[0]["TransactionID"].ToString() + " Thank You ";
                                string sendURL = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-hsrphr&password=hsrphr&type=0&dlr=1&destination=" + MobileNo.ToString() + "&source=HRHSRP&message=" + SMSText;
                                HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create(sendURL);
                                myRequest.Method = "GET";
                                WebResponse myResponse = myRequest.GetResponse();
        ...
        public static List<string> SearchCustomers(string prefixText, int count)
        {
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                {
                    cmd.CommandText = "select distinct bt.DealerID+','+' '+dm.dealername as dealer from dealermaster dm join BankTransaction bt on bt.dealerid=dm.dealerid where   bt.DealerID like @SearchText + '%'";
        ...
        public static List<string> SearchByDealerName(string prefixText, int count)
        {
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct (dm.dealername +' '+','+ CONVERT(varchar,dm.DealerID))  as dealer from dealermaster dm join BankTransaction bt on bt.dealerid=dm.dealerid where  dm.dealername like @SearchText + '%'";
                    cmd.Parameters.AddWithValue("@SearchText", prefixText);
                    cmd.Connection = conn;
                    conn.Open();
                    List<string> customers = new List<string>();
