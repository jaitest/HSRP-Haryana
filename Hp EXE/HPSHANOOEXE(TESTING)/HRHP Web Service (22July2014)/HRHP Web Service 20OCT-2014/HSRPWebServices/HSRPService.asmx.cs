...
namespace HSRPWebServices
{
    ...
    public class HSRPService : System.Web.Services.WebService
    {
        ...
        #region GetDataForGST
        [WebMethod]
        public DataTable GetGSTDetail(string Vehicleregno)
        {
            string StrRecord = string.Empty;


            string sqlText = "select top 1    SGSTPer,	SGSTAmount,	CGSTPer	,CGSTAmount	, gstbasicamount from HSRPRecords where vehicleregno ='" + Vehicleregno.ToString() + "' and hsrp_stateid =3  order by   hsrprecord_creationdate desc ";

            string CnnString = obj.strProvider;
            DataTable dt = new DataTable();
            dt = utils.GetDataTable(sqlText, CnnString);

            dt.TableName = "HSRPRecords_GSTbihar";
        ...
        # region Chalan
        [WebMethod]
        public DataTable Chalan()
        {
            string CnnString = obj.strProvider;
            string sqlText = "select * from HSRPState WHERE HSRP_StateID='3'";
            dt1 = utils.GetDataTable(sqlText, CnnString);
        ...
        {
            string CnnString = obj.strProvider;
            string sqlText = "insert into [hsrpdemo].[dbo].[SMSlog_HP]([hsrp_stateid],SMSLogID,[MobileNo],[REGN_NO],[AUTH_NO],[HSRP_FLAG],[CashReceiptSmsText],[CashReceiptSMSDateTime],[CashReceiptSMSServerResponseID],[CashReceiptSMSServerResponseText]) values" +
                               "('" + strHSRP_StateId.Replace(" ", "") + "', (select  top 1  SMSLogID    from  SMSlog_HP order by  [CashReceiptSMSDateTime] desc)+1  , '" + strMobileNo.Replace(" ", "") + "','" + strRegNo.Replace(" ", "") + "','" + strAuthNo + "','" + strHsrp_Flag + "','" + strSmsText + "',getdate(),'" + smsresponseid + "','" + smsresponsetext + "')";
            try
            {
